...
use crate::event_queue::{DigNodeIdx, EventQueue};
...
impl AdcBridge {
    pub fn new(analog_node: u32, digital_node: DigNodeIdx, in_low: f64, in_high: f64) -> Self {
        Self {
            analog_node,
            digital_node,
            in_low,
    ...
    pub fn step(&mut self, t: f64, voltage: f64, queue: &mut EventQueue) -> bool {
        ...
        if let Some(s) = new_state {
            if s != self.last_state {
                queue.schedule(t, self.digital_node, s);
                self.last_state = s;
                return true;
            }
        }
...
mod tests {
    ...
    fn adc_emits_high_above_threshold() {
        let mut adc = AdcBridge::new(0, DigNodeIdx::new(7), 1.0, 2.0);
