...
impl WgpuBackend {
    ...
    /// (e.g. `incspice_device::bsim4::gpu`) that need to compile a
    ...
    fn gpu_axpy_or_scale(
        ...
        let y_buf = if let Some(y) = y_opt {
            ...
            self.create_buffer(&vec![0u8; x_bytes.len()], wgpu::BufferUsages::STORAGE)
        };

        let result_buf = self.create_buffer(&vec![0u8; 4], wgpu::BufferUsages::STORAGE);
...
impl ComputeBackend for WgpuBackend {
    ...
    fn norm_inf(&self, x: &[f64]) -> f64 {
        ...
        partials.iter().map(|&v| v as f64).fold(0.0f64, f64::max)
    }
