...
/// CPU backend using auto-vectorizable scalar loops and rayon.
...
fn simd_norm_inf(x: &[f64]) -> f64 {
    x.iter().copied().fold(0.0f64, |acc, v| acc.max(v.abs()))
}
...
pub enum Backend {
    ...
    #[cfg(feature = "gpu")]
    Gpu(super::gpu_backend::WgpuBackend),
}
...
impl Backend {
    ...
    pub fn gpu(min_gpu_size: usize) -> Option<Self> {
        super::gpu_backend::WgpuBackend::new(min_gpu_size).map(Self::Gpu)
    }
}
