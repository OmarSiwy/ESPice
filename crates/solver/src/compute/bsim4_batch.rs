...
//
// Public API:
//   eval_bsim4_batch(&Bsim4BatchInput, &dyn ComputeBackend) -> Result<Bsim4BatchOutput, ComputeError>
//
...
/// Errors from batch device evaluation.
#[derive(Debug, Error)]
...
fn eval_one(vgs_in: f32, vds_in: f32, vbs_in: f32, p: &GpuBsim4Params) -> GpuBsim4Out {
    ...
    let abulk = fmax(
        p.a0 * (1.0 + p.ags * vgsteff) * (1.0 - p.keta * vbs_eff),
        1.0e-3,
    );
    ...
    let gds =
        ids * (dvdseff_dvd * inv_vdseff) + ids_core * (clm - 1.0) / fmax(p.leff, 1.0e-9) * 1.0e-9;
...
pub fn eval_bsim4_batch_cpu(input: &Bsim4BatchInput) -> Result<Bsim4BatchOutput, ComputeError> {
    let n = input.vgs.len();
    if input.vds.len() != n || input.vbs.len() != n {
        return Err(ComputeError::Compute(format!(
...
pub fn eval_bsim4_batch(
    ...
    backend: &dyn super::backend::ComputeBackend,
) -> Result<Bsim4BatchOutput, ComputeError> {
