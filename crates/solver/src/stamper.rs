use incspice_core::{Circuit, DeviceKind, LtraHistoryStore};
use crate::device::{DeviceEval, DeviceRegistry, eval_bsource_i, eval_bsource_v};
use crate::device::{LtraLineParams, LtraNorton, eval_ltra_transient_slices_nonint};
use crate::linalg::{DenseVec, TripletMatrix};
use rayon::prelude::*;
...
fn pin_to_global(device: &incspice_core::DeviceInstance, pin: u8, num_vars: u32) -> Option<usize> {
...
pub fn stamp_circuit_into(
    ...
    for device in circuit.devices() {
        ...
        if device.kind == DeviceKind::Tline {
            ...
            // Terminal voltages.
            let vp1 = rp1
                .map(|r| if r < solution.len() { solution[r] } else { 0.0 })
                .unwrap_or(0.0);
            let vn1 = rn1
                .map(|r| if r < solution.len() { solution[r] } else { 0.0 })
                .unwrap_or(0.0);
            ...
            // Branch currents.
            let i_br1 = if br1 < solution.len() {
                solution[br1]
            } else {
                0.0
            };
            let i_br2 = if br2 < solution.len() {
                solution[br2]
            } else {
                0.0
            };
        ...
        if matches!(
            ...
            let is_voltage_form =
                matches!(device.kind, DeviceKind::BsourceV | DeviceKind::VcvsExpr);
            // For voltage-form B-sources, branch_index must be set; skip if missing.
            let bsource_br_idx_opt: Option<usize> = if is_voltage_form {
                match device.branch_index {
                    Some(bi) => Some(circuit.num_vars() as usize + bi as usize),
                    None => continue,
                }
            } else {
                None
            ...
            let eval_result = if is_voltage_form {
                let br_idx = bsource_br_idx_opt.unwrap(); // safe: guarded above
                let i_branch = if br_idx < solution.len() {
                    solution[br_idx]
                } else {
                    0.0
                };
            ...
            let np_row = if device.terminals[0].node.is_ground() {
                None
            } else {
            ...
            if is_voltage_form {
                ...
                if let Some(r) = np_row {
                    residual[r] += eval.g[0];
                }
                if let Some(r) = nn_row {
                    residual[r] += eval.g[1];
                }
                ...
                let br_row = bsource_br_idx_opt.unwrap(); // safe: guarded above
            ...
            let (base_ref_pin, branch_row_opt) = if is_voltage_form {
                let br = bsource_br_idx_opt.unwrap(); // safe: guarded above
                (3u8, Some(br))
            } else {
                (2u8, None)
            };
        ...
        for term in &device.terminals {
            if term.node.is_ground() {
                ...
                voltages.push(if idx < solution.len() {
                    solution[idx]
                } else {
                    0.0
                });
        ...
        // Safety guard: skip devices whose terminal count doesn't match the model.
        // This happens when the parser adds unsupported elements (K, A, O, etc.)
        // with empty terminal lists. Without this guard, voltages[0] would panic.
        if voltages.len() < model.num_terminals() {
            continue;
        }
        ...
        let cswitch_params_buf: Option<incspice_core::ParamMap> =
            if device.kind == DeviceKind::CSwitch {
                device.params.get("ctrl_branch_index").map(|ctrl_bi| {
                    ...
                    let ic = if ctrl_br_idx < solution.len() {
                        solution[ctrl_br_idx]
                    } else {
                        0.0
                    };
        ...
        let eval: DeviceEval = if device.needs_branch() {
            let bi = match device.branch_index {
                Some(b) => b,
                None => continue, // branch index missing — skip device
            };
            let br_idx = circuit.num_vars() as usize + bi as usize;
            let i_branch = if br_idx < solution.len() {
                solution[br_idx]
            } else {
                0.0
...
pub fn stamp_circuit_gc_into(
    ...
    stamp_circuit_gc_at_time(
        circuit, solution, registry, g_triplet, c_triplet, residual_g, residual_q, 0.0, None,
    );
}
...
pub fn stamp_circuit_gc_at_time(
    ...
    for device in circuit.devices() {
        ...
        if device.kind == DeviceKind::Tline {
            ...
            // Stamp G Jacobian.
            if let Some(r) = rp1 {
                g_triplet.add(r, br1, 1.0);
            }
            if let Some(r) = rn1 {
                g_triplet.add(r, br1, -1.0);
            }
            if let Some(r) = rp2 {
                g_triplet.add(r, br2, 1.0);
            }
            if let Some(r) = rn2 {
                g_triplet.add(r, br2, -1.0);
            }
            if let Some(r) = rp1 {
                g_triplet.add(br1, r, 1.0);
            }
            if let Some(r) = rn1 {
                g_triplet.add(br1, r, -1.0);
            }
        ...
        if device.kind == DeviceKind::Ltra {
            ...
            let pin_row =
                |pin: u8| -> Option<usize> { pin_to_global(device, pin, circuit.num_vars()) };
            ...
            if use_companion {
                ...
                if let Some(r) = row_p1 {
                    residual_g[r] += y0 * v1_diff + norton.i_eq_p1;
                }
                if let Some(r) = row_n1 {
                    residual_g[r] += -(y0 * v1_diff + norton.i_eq_p1);
                }
                if let Some(r) = row_p2 {
                    residual_g[r] += y0 * v2_diff + norton.i_eq_p2;
                }
                if let Some(r) = row_n2 {
                    residual_g[r] += -(y0 * v2_diff + norton.i_eq_p2);
                }

                // Jacobian: dI/dV — Y0 self-admittance at each port,
                // no cross-port terms (coupling is through the Norton current).
                if let (Some(r), Some(c)) = (row_p1, row_p1) {
                    g_triplet.add(r, c, y0);
                }
                if let (Some(r), Some(c)) = (row_p1, row_n1) {
                    g_triplet.add(r, c, -y0);
                }
                if let (Some(r), Some(c)) = (row_n1, row_p1) {
                    g_triplet.add(r, c, -y0);
                }
                if let (Some(r), Some(c)) = (row_n1, row_n1) {
                    g_triplet.add(r, c, y0);
                }
                if let (Some(r), Some(c)) = (row_p2, row_p2) {
                    g_triplet.add(r, c, y0);
                }
                if let (Some(r), Some(c)) = (row_p2, row_n2) {
                    g_triplet.add(r, c, -y0);
                }
                if let (Some(r), Some(c)) = (row_n2, row_p2) {
                    g_triplet.add(r, c, -y0);
                }
                if let (Some(r), Some(c)) = (row_n2, row_n2) {
                    g_triplet.add(r, c, y0);
                }
            } else {
                ...
                if let Some(r) = row_p1 {
                    residual_g[r] += i_series + i_sh1;
                }
                if let Some(r) = row_n1 {
                    residual_g[r] += -i_sh1;
                }
                if let Some(r) = row_p2 {
                    residual_g[r] += -i_series + i_sh2;
                }
                if let Some(r) = row_n2 {
                    residual_g[r] += -i_sh2;
                }
                ...
                let gh = g_shunt;
                if let (Some(r), Some(c)) = (row_p1, row_p1) {
                    g_triplet.add(r, c, gs + gh);
                }
                if let (Some(r), Some(c)) = (row_p1, row_n1) {
                    g_triplet.add(r, c, -gh);
                }
                if let (Some(r), Some(c)) = (row_p1, row_p2) {
                    g_triplet.add(r, c, -gs);
                }
                if let (Some(r), Some(c)) = (row_n1, row_p1) {
                    g_triplet.add(r, c, -gh);
                }
                ...
                if let (Some(r), Some(c)) = (row_p2, row_p1) {
                    g_triplet.add(r, c, -gs);
                }
                if let (Some(r), Some(c)) = (row_p2, row_p2) {
                    g_triplet.add(r, c, gs + gh);
        ...
        // Safety guard: skip devices with wrong terminal count.
        if voltages.len() < model.num_terminals() {
            continue;
        }

        // Apply per-device junction voltage limiting when previous solution
        // is available (same logic as stamp_circuit_into).
        if let Some(prev) = prev_solution {
            let mut old_voltages: smallvec::SmallVec<[f64; 4]> = smallvec::SmallVec::new();
            for term in &device.terminals {
                if term.node.is_ground() {
        ...
        let cswitch_params_buf_gc: Option<incspice_core::ParamMap> =
            if device.kind == DeviceKind::CSwitch {
                device.params.get("ctrl_branch_index").map(|ctrl_bi| {
                    let ctrl_br_idx = circuit.num_vars() as usize + ctrl_bi as usize;
                    let ic = if ctrl_br_idx < solution.len() {
    ...
    for &(id_l1, id_l2, k) in circuit.mutual_couplings() {
        ...
        let i1 = if l1_br_row < solution.len() {
            solution[l1_br_row]
        } else {
            0.0
        };
        let i2 = if l2_br_row < solution.len() {
            solution[l2_br_row]
        } else {
            0.0
        };
...
pub fn stamp_circuit_with_source_scale(
    ...
    for device in circuit.devices() {
        ...
        // Inject controlling branch current for CSwitch (W element).
        let cswitch_params_buf_ss: Option<incspice_core::ParamMap> =
            if device.kind == DeviceKind::CSwitch {
                device.params.get("ctrl_branch_index").map(|ctrl_bi| {
                    let ctrl_br_idx = circuit.num_vars() as usize + ctrl_bi as usize;
                    let ic = if ctrl_br_idx < solution.len() {
...
pub fn stamp_circuit_gc_par_at_time(
        ...
        match device.kind {
            ...
            DeviceKind::Ltra => {
                ...
                if use_companion {
                    ...
                    let v2d = vp2 - vn2;
                    if let Some(r) = row_p1 {
                        residual_g[r] += y0 * v1d + norton.i_eq_p1;
                    }
                    if let Some(r) = row_n1 {
                        residual_g[r] += -(y0 * v1d + norton.i_eq_p1);
                    }
                    if let Some(r) = row_p2 {
                        residual_g[r] += y0 * v2d + norton.i_eq_p2;
                    }
                    if let Some(r) = row_n2 {
                        residual_g[r] += -(y0 * v2d + norton.i_eq_p2);
                    }
            ...
            | DeviceKind::VccsExpr => {
                ...
                let is_voltage_form =
                    matches!(device.kind, DeviceKind::BsourceV | DeviceKind::VcvsExpr);
                // For voltage-form B-sources, branch_index must be set; skip if missing.
                let par_bsource_br_idx_opt: Option<usize> = if is_voltage_form {
                    match device.branch_index {
                        Some(bi) => Some(circuit.num_vars() as usize + bi as usize),
                        None => continue,
                    }
                ...
                let eval_result = if is_voltage_form {
                    let br_idx = par_bsource_br_idx_opt.unwrap(); // safe: guarded above
                    let i_branch = if br_idx < solution.len() {
                        solution[br_idx]
                    } else {
                        0.0
                ...
                if is_voltage_form {
                    if let Some(r) = np_row {
                        residual_g[r] += eval.g[0];
                    }
                    if let Some(r) = nn_row {
                        residual_g[r] += eval.g[1];
                    }
                    let br_row = par_bsource_br_idx_opt.unwrap(); // safe: guarded above
                ...
                let (base_ref_pin, branch_row_opt) = if is_voltage_form {
                    let br = par_bsource_br_idx_opt.unwrap(); // safe: guarded above
                    (3u8, Some(br))
                } else {
                    (2u8, None)
                };
    ...
    struct ParEvalInput {
        ...
        branch_current: f64,
        params_override: Option<incspice_core::ParamMap>,
    }
        ...
        .filter(|(_, dev)| {
            !matches!(
                ...
                DeviceKind::Tline
                    ...
                    | DeviceKind::VcvsExpr
                    | DeviceKind::VccsExpr
            ) && registry.get(dev.kind).map_or(false, |m| {
                dev.terminals.len() >= m.num_terminals()
            })
        })
        .map(|(i, dev)| {
            let mut voltages: smallvec::SmallVec<[f64; 4]> = smallvec::SmallVec::new();
            ...
            let branch_current = if has_branch {
                match dev.branch_index {
                    Some(bi) => {
                        let br_idx = num_vars as usize + bi as usize;
                        if br_idx < solution.len() {
                            solution[br_idx]
                        } else {
                            0.0
                        }
                    }
                    None => 0.0, // branch index missing — use 0 (will be skipped in stamp)
                }
            } else {
                0.0
            };
...
pub fn update_tline_histories(circuit: &mut Circuit, solution: &[f64], time: f64) {
    ...
    let mut updates: Vec<(incspice_core::DeviceId, f64, f64, f64, f64)> = Vec::new();
    ...
    for (id, e_p1, e_p2, z0, td) in updates {
        ...
        if circuit.tline_history(id).is_none() {
            ...
            circuit.add_tline_history(id, incspice_core::TlineHistory::new(z0, td, 256));
        }
...
mod tests {
    ...
    #[test]
    fn test_stamper_missing_branch_returns_error() {
        // Circuit::validate() detects devices that need a branch but have
        // branch_index = None.  We test this via the validate() API which is
        // accessible from outside the core crate, and verify the stamper
        // does not panic on a well-formed circuit.

        // Build a well-formed circuit via add_device (assigns branch_index).
        let ckt = voltage_divider();
        // A properly built circuit should pass validation.
        assert!(
            ckt.validate().is_ok(),
            "well-formed circuit should pass validation"
        );

        // Build a circuit where validate() returns Err: use a VoltageSource
        // that we then inspect.  Since add_device always assigns branch_index
        // for devices that need_branch(), we test validate() via core tests.
        // Here we confirm the stamper runs without panic on a valid circuit.
        let reg = crate::device::DeviceRegistry::new_default();
        let dim = ckt.mna_dimension();
        let solution = vec![0.0_f64; dim];
        // Must not panic:
        let (_jac, _res) = stamp_circuit(dim, &ckt, &solution, &reg);
    }

    #[test]
    fn update_tline_histories_pushes_wave_observables() {
        // Build a minimal T-line circuit: T1 from n1 to n2, Z0=50, TD=1ns.
        // Solution: V(n1)=1.0, V(n2)=0.0, I_br1=0.02 (= 1V/50Ω), I_br2=-0.02.
        let mut ckt = Circuit::new();
        let n1 = ckt.add_node("p1");
        let n2 = ckt.add_node("p2");

        let tline = DeviceInstance::new(
            DeviceId::new(0),
            "T1",
            DeviceKind::Tline,
        ...
        let id = ckt.add_device(tline);
        // 2 nodes + 2 branch currents (Tline uses 2 branches).
        // solution = [V(n1), V(n2), I_br1, I_br2]
        let solution = vec![1.0_f64, 0.0, 0.02, -0.02];

        update_tline_histories(&mut ckt, &solution, 1e-10);

        let hist = ckt.tline_history(id).expect("missing TlineHistory after update");
        // E_p1 = V(n1) - 0 + Z0*I_br1 = 1.0 + 50*0.02 = 2.0
        // E_p2 = V(n2) - 0 + Z0*I_br2 = 0.0 + 50*(-0.02) = -1.0
        let e_p1 = hist.delayed_p1(1e-10 + 1e-9); // look up at t=1.1ns (>= td)
        let e_p2 = hist.delayed_p2(1e-10 + 1e-9);
        // The delayed lookup should return the value we just pushed at t=1e-10.
        assert!(
            (e_p1 - 2.0).abs() < 1e-10,
            "e_p1 delayed value should be ~2.0, got {e_p1}"
        );
        assert!(
            (e_p2 - (-1.0)).abs() < 1e-10,
            "e_p2 delayed value should be ~-1.0, got {e_p2}"
        );
    }

    #[test]
    fn update_ltra_histories_stores_accepted_axial_currents() {
        let mut ckt = Circuit::new();
        let n1 = ckt.add_node("p1");
        let n2 = ckt.add_node("p2");

        let ltra = DeviceInstance::new(
            DeviceId::new(0),
            "O1",
            DeviceKind::Ltra,
            &[(0, n1), (1, NodeId::GROUND), (2, n2), (3, NodeId::GROUND)],
        ...
        let id = ckt.add_device(ltra);
        let solution = vec![1.0, 0.0];

        update_ltra_histories(&mut ckt, &solution, 1.0);

        let hist = ckt.ltra_history(id).expect("missing LTRA history");
        assert_eq!(hist.times, vec![1.0]);
        assert_eq!(hist.v1, vec![1.0]);
        assert_eq!(hist.v2, vec![0.0]);
        assert!((hist.i1[0] - 0.02).abs() < 1e-12, "i1={}", hist.i1[0]);
        assert!((hist.i2[0] + 0.02).abs() < 1e-12, "i2={}", hist.i2[0]);
    }
}

/// Record the accepted port voltages and axial currents for every LTRA
/// element into the circuit's `LtraHistoryStore`.
///
/// Called by the transient driver after each accepted timestep, before
/// advancing to the next. The stored `V`/`I` samples are consumed by the
/// convolution path during the next Newton iteration to reconstruct the
/// delayed wave observable `E = V + Z0*I`.
pub fn update_ltra_histories(circuit: &mut Circuit, solution: &[f64], time: f64) {
    ...
    for device in circuit.devices() {
        ...
        let (i1, i2) = if let Some(hist) = circuit.ltra_history(device.id) {
            ...
            if lp.l >= 1e-30 && norton.g_eq_p1 > 0.0 {
                (
                    norton.g_eq_p1 * v1 + norton.i_eq_p1,
                    norton.g_eq_p2 * v2 + norton.i_eq_p2,
                )
            } else {
                let r_tot = (lp.r * lp.len).max(0.0);
                let r_eff = if r_tot > 0.0 { r_tot } else { 1.0 / 1e-12_f64 };
                let i_series = (v1 - v2) / r_eff;
                (i_series, -i_series)
            }
        } else {
            let r_tot = (lp.r * lp.len).max(0.0);
