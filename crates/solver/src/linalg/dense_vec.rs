...
mod tests {
    ...
    #[test]
    fn test_norm_inf_all_positive() {
        let v = DenseVec::from_slice(&[1.0, 5.0, 3.0, 2.0]);
        assert_eq!(v.norm_inf(), 5.0);
    }

    #[test]
    fn test_norm_inf_all_negative() {
        let v = DenseVec::from_slice(&[-1.0, -5.0, -3.0]);
        assert_eq!(v.norm_inf(), 5.0);
    }

    #[test]
    fn test_norm2_known_triple() {
    ...
    #[test]
    fn test_norm2_single_element() {
        let v = DenseVec::from_slice(&[-7.0]);
        assert!((v.norm2() - 7.0).abs() < 1e-14);
    }

    #[test]
    fn test_axpy_zero_scalar() {
        let mut y = DenseVec::from_slice(&[1.0, 2.0, 3.0]);
        let x = DenseVec::from_slice(&[100.0, 200.0, 300.0]);
        y.axpy(0.0, &x);
        assert_eq!(y[0], 1.0);
        assert_eq!(y[1], 2.0);
        assert_eq!(y[2], 3.0);
    ...
    #[test]
    fn test_axpy_negative_scalar() {
        let mut y = DenseVec::from_slice(&[5.0, 10.0]);
        let x = DenseVec::from_slice(&[2.0, 4.0]);
        y.axpy(-1.0, &x);
    ...
    fn test_dot_orthogonal() {
        let a = DenseVec::from_slice(&[1.0, 0.0]);
        let b = DenseVec::from_slice(&[0.0, 1.0]);
        assert!((a.dot(&b)).abs() < 1e-14);
    }
    ...
    fn test_scale_by_negative() {
        let mut v = DenseVec::from_slice(&[1.0, -2.0]);
        v.scale(-1.0);
        assert_eq!(v[0], -1.0);
        assert_eq!(v[1], 2.0);
    }
    ...
    #[test]
    fn test_sub_produces_self_minus_other() {
        let a = DenseVec::from_slice(&[5.0, 3.0]);
        let b = DenseVec::from_slice(&[2.0, 1.0]);
        let c = &a - &b;
        assert_eq!(c[0], 3.0);
        assert_eq!(c[1], 2.0);
    }
    ...
    fn test_as_slice_roundtrip() {
        ...
        let v = DenseVec::from_slice(&data);
        assert_eq!(v.as_slice(), &data);
    }

    #[test]
    fn test_fill_zero_idempotent() {
        let mut v = DenseVec::zeros(5);
        v.fill_zero(); // already zero — should stay zero
        assert_eq!(v.norm_inf(), 0.0);
    }
    ...
    fn test_large_vector_dot() {
        let n = 100;
        let a = DenseVec::from_slice(&vec![1.0; n]);
        let b = DenseVec::from_slice(&vec![2.0; n]);
        assert!((a.dot(&b) - 200.0).abs() < 1e-10);
    }
}
