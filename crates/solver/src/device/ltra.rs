...
impl DeviceModel for Ltra {
    ...
    fn eval_with_branch(
        ...
        //   pin 3 (n2-): -I_sh2
        let g = smallvec![i_series + i_sh1, -i_sh1, -i_series + i_sh2, -i_sh2,];
...
pub fn ltra_kernel(tau: f64, lp: &LtraLineParams) -> f64 {
    ...
    match lp.regime() {
        ...
        LtraRegime::RcDominated => {
            ...
            // At tau == 0 the kernel has a singularity: the t^(-3/2) power law
            // diverges while the Gaussian factor exp(-A/tau) → 0, giving 0*∞ = NaN.
            // Physically, the kernel peaks at tau_peak = 2*A / 3 where
            // A = rc_pu * len² / 4.  Evaluate at tau_peak when tau == 0 so that
            // callers see a well-defined, large positive value at the singularity.
            let effective_tau = if tau == 0.0 {
                let a = rc_pu * lp.len * lp.len / 4.0;
                2.0 * a / 3.0
            } else {
                tau
            };
            norm * effective_tau.powf(-1.5)
                * (-rc_pu * lp.len * lp.len / (4.0 * effective_tau)).exp()
        }
        LtraRegime::General => {
            let td = lp.td();
            if tau < td {
...
/// current comes from the *opposite* port's accepted wave observable
/// `E = V + Z0*I` delayed by TD and attenuated by `A = exp(-alpha_neper)`.
///
...
pub fn compute_norton_equivalent(
    ...
    if lp.l < 1e-30 {
        ...
        for k in 0..n - 1 {
            ...
            // tau_k1 == 0 corresponds to the current timestep; the kernel at
            // tau == 0 is evaluated at tau_peak (see ltra_kernel), which is a
            // large finite value — safe to include in the quadrature.
            let h_k1 = ltra_kernel(tau_k1, lp);
            // For RC-dominated lines there is no wave propagation: each port's
            // Norton current depends on that port's OWN voltage history via the
            // RC diffusion self-response (not the opposite port's history as in
            // the Branin LC model).
            i_eq_p1 += 0.5 * (h_k * v1[k] + h_k1 * v1[k + 1]) * dt;
            i_eq_p2 += 0.5 * (h_k * v2[k] + h_k1 * v2[k + 1]) * dt;
        }
        return LtraNorton {
            g_eq_p1: 0.0,
            g_eq_p2: 0.0,
    ...
    // Reconstruct the delayed accepted wave observables E = V + Z0*I
    // from the stored port voltages and axial currents.
    let e1_delayed = if t_delayed >= times[0] {
        interp_history(times, v1, t_delayed) + z0 * interp_history(times, i1, t_delayed)
    } else {
        0.0
    };
    let e2_delayed = if t_delayed >= times[0] {
        interp_history(times, v2, t_delayed) + z0 * interp_history(times, i2, t_delayed)
    } else {
        0.0
    };
...
pub fn compute_norton_equivalent_nonint(
    ...
    // Reconstruct the delayed accepted wave observables E = V + Z0*I
    // using nearest-neighbour reads for both stored fields.
    let e1_delayed = if t_delayed >= times[0] {
...
mod tests {
    ...
    fn ltra_jacobian_symmetry() {
        ...
        for &(r, c, v) in &eval.G {
            let found = eval
                .G
                .iter()
                .any(|&(rr, cc, vv)| rr == c && cc == r && (vv - v).abs() < 1e-15);
    ...
    fn general_kernel_zero_before_td() {
        ...
        assert!(
            h_before.abs() < 1e-30,
            "h before TD should be 0, got {h_before}"
        );
        // And positive at TD.
    ...
    fn rc_norton_nonzero_with_history() {
        ...
        assert!(
            n.i_eq_p1 > 0.0,
            "I_eq_p1 should be positive with 1V history"
        );
    }

    #[test]
    fn general_norton_reconstructs_wave_from_stored_current_history() {
        let lp = LtraLineParams {
            r: 0.0,
            l: 2.0,
            g: 0.0,
            c: 0.5,
        ...
        let n = compute_norton_equivalent(2.0, &lp, &times, &v1, &v2, &i1, &i2);

        assert!((n.g_eq_p1 - 0.5).abs() < 1e-12);
        assert!((n.i_eq_p1 + 3.0).abs() < 1e-12);
        assert!((n.i_eq_p2 + 2.5).abs() < 1e-12);
    }
}
