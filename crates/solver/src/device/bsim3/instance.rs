...
impl Bsim3SizeParams {
    ...
    pub fn resolve(model: &Bsim3Model, inst: &ParamMap) -> Self {
        ...
        let l_pow = if p.lln != 0.0 {
            l_drawn.powf(p.lln)
        } else {
            1.0
        };
        let w_pow = if p.lwn != 0.0 {
            w_drawn.powf(p.lwn)
        } else {
            1.0
        };
        let dl = p.lint + p.ll / l_pow + p.lw / w_pow + p.lwl / (l_pow * w_pow);

        let l_pow_w = if p.wln != 0.0 {
            l_drawn.powf(p.wln)
        } else {
            1.0
        };
        let w_pow_w = if p.wwn != 0.0 {
            w_drawn.powf(p.wwn)
        } else {
            1.0
        };
        let dw = p.wint + p.wl / l_pow_w + p.ww / w_pow_w + p.wwl / (l_pow_w * w_pow_w);
        ...
        let ni_si = 1.45e10 * (temp / 300.15).powf(1.5) * (21.5565981 - eg / (2.0 * vt_t)).exp();
        ...
        let theta0vb0 =
            ((-p.dvt1 * 0.5 * leff / l_t).exp() + 2.0 * (-p.dvt1 * leff / l_t).exp()).max(0.0);
        let thetaRout =
            ((-p.drout * 0.5 * leff / l_t).exp() + 2.0 * (-p.drout * leff / l_t).exp()).max(0.0);
