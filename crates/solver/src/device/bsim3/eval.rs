...
use super::instance::{Bsim3SizeParams, consts};
...
pub fn compute_vth(p: &Bsim3SizeParams, vbseff: f64, vds: f64) -> (f64, f64, f64, f64) {
    ...
    let vth_body = p.vth0 + p.k1ox * (sqrt_phis - p.sqrtPhi) - p.k2ox * vbseff;
...
pub fn compute_n(p: &Bsim3SizeParams, vbseff: f64, sqrt_phis: f64) -> f64 {
    ...
    let term = p.nfactor * cdep / p.cox + (p.cdsc + p.cdscb * vbseff) / p.cox + p.cit / p.cox;
    (1.0 + term).max(1.0)
}

#[inline]
pub fn compute_vgsteff(p: &Bsim3SizeParams, vgs: f64, vth: f64, n: f64) -> (f64, f64, f64) {
...
// ────────────────────────────────────────────────────────────────────────
#[inline]
pub fn compute_mobility(p: &Bsim3SizeParams, vgsteff: f64, vbseff: f64) -> (f64, f64, f64) {
    let tox = p.tox;
