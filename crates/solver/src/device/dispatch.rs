...
use crate::device::bsource::{BsourceIModel, BsourceVModel};
use crate::device::eval::{DeviceEval, DeviceModel};
use crate::device::ltra::Ltra;
use crate::device::mosfet::{MosfetLevel2, MosfetLevel3, MosfetLevel6};
use crate::device::port::Port;
use crate::device::tline::Tline;
use crate::device::urc::Urc;
use crate::device::vccs::VccsExpr;
use crate::device::vcvs::VcvsExpr;
use crate::device::wlossy::WLossy;
use crate::device::{
    Bjt, Bsim3, Bsim4, CSwitch, Capacitor, Cccs, Ccvs, CurrentSource, Diode, Inductor, JfetLevel1,
    Mesfet, MosfetLevel1, Resistor, Switch, Vbic, Vccs, Vcvs, VoltageSource,
};
...
pub enum DeviceDispatch {
    ...
    /// is performed by `incspice_osdi::OsdiRegistry`, which the stamper
    /// borrows alongside the dispatch enum. Keeping the payload as POD
    /// avoids a crate-graph cycle (`incspice-osdi` depends on
    /// `incspice-device`, not the other way around).
    Osdi(crate::device::osdi_shim::OsdiHandle),
...
impl DeviceDispatch {
    ...
    pub fn eval(&self, voltages: &[f64], params: &ParamMap) -> DeviceEval {
        match self {
            ...
            // `incspice_osdi::OsdiRegistry::evaluate`. Returning an empty
    ...
    pub fn kind(&self) -> DeviceKind {
        match self {
            ...
            DeviceDispatch::Osdi(_) => crate::device::osdi_shim::OSDI_KIND_SENTINEL,
    ...
    pub fn eval_with_branch(
        ...
        match self {
            ...
            DeviceDispatch::VoltageSource(m) => {
                m.eval_with_branch(voltages, branch_current, params)
            }
...
mod tests {
    ...
    fn dispatch_num_terminals() {
        ...
        assert_eq!(
            DeviceDispatch::MosfetLevel1(MosfetLevel1).num_terminals(),
            4
        );
        assert_eq!(
            DeviceDispatch::VoltageSource(VoltageSource).num_terminals(),
            2
        );
        assert_eq!(
            DeviceDispatch::CurrentSource(CurrentSource).num_terminals(),
            2
        );
    ...
    fn dispatch_kind() {
        assert_eq!(
            DeviceDispatch::Resistor(Resistor).kind(),
            DeviceKind::Resistor
        );
        assert_eq!(
            DeviceDispatch::VoltageSource(VoltageSource).kind(),
