...
pub use eval::{Bsim4Eval, evaluate_dc};
...
mod tests {
    ...
    fn bsim4_nmos_cutoff_yields_small_current() {
        ...
        assert!(
            e.g[0].abs() < 1.0e-6,
            "Id at Vgs=0 should be sub-threshold, got {}",
            e.g[0]
        );
    }
    ...
    fn bsim4_nmos_saturation_positive_id() {
        ...
        assert!(
            e.g[0] > 0.0,
            "Id should be positive in saturation, got {}",
            e.g[0]
        );
        // KCL: Id + Is + Ig + Ib = 0
    ...
    fn bsim4_pmos_polarity() {
        ...
        assert!(
            e.g[0] <= 1.0e-9,
            "PMOS Id should be ≤ 0 in forward bias, got {}",
            e.g[0]
        );
    }
}
