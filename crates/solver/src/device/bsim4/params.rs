...
pub struct Bsim4ModelParams {
    ...
    pub cjs: f64,   // bottom junction cap per unit area (source) [F/m^2]
    pub cjd: f64,   // bottom junction cap per unit area (drain)  [F/m^2]
    pub pbs: f64,   // source junction built-in potential [V]
    pub pbd: f64,   // drain  junction built-in potential [V]
    pub mjs: f64,   // source junction grading coefficient
    pub mjd: f64,   // drain  junction grading coefficient
...
impl Default for Bsim4ModelParams {
    fn default() -> Self {
        ...
        Self {
            ...
            cjs: 5.0e-4,  // BSIM4 default: 5e-4 F/m^2
            cjd: 5.0e-4,
            pbs: 1.0,
            pbd: 1.0,
            mjs: 0.5,
...
impl Bsim4ModelParams {
    ...
    pub fn from_map(map: &ParamMap, mos_type: Bsim4Type) -> Self {
        ...
        if !(0..=6).contains(&p.flags.mobMod) {
            log::warn!(
                "BSIM4: mobMod={} out of range [0,6], falling back to default {}",
                p.flags.mobMod,
                d.flags.mobMod
            );
            p.flags.mobMod = d.flags.mobMod;
        }
        if !(0..=2).contains(&p.flags.capMod) {
            log::warn!(
                "BSIM4: capMod={} out of range [0,2], falling back to default {}",
                p.flags.capMod,
                d.flags.capMod
            );
            p.flags.capMod = d.flags.capMod;
        }
        if !(0..=2).contains(&p.flags.diomod) {
            log::warn!(
                "BSIM4: dioMod={} out of range [0,2], falling back to default {}",
                p.flags.diomod,
                d.flags.diomod
            );
            p.flags.diomod = d.flags.diomod;
        }
        ...
        p.aigc = g(
            "aigc",
            if matches!(mos_type, Bsim4Type::Nmos) {
                d.aigbacc
            } else {
                d.aigbinv
            },
        ...
        if map.get("toxe").is_none() {
            log::debug!("BSIM4: toxe not specified, using default {:.3e} m", p.toxe);
        }
        if map.get("vth0").is_none() && map.get("vto").is_none() {
            log::debug!("BSIM4: vth0 not specified, using default {:.3} V", p.vth0);
        }
        if map.get("u0").is_none() {
            log::debug!("BSIM4: u0 not specified, using default {:.4} m^2/V/s", p.u0);
        }
        if map.get("vsat").is_none() {
            log::debug!("BSIM4: vsat not specified, using default {:.3e} m/s", p.vsat);
        }
        if map.get("ndep").is_none() {
            log::debug!("BSIM4: ndep not specified, using default {:.3e} cm^-3", p.ndep);
        }
        if map.get("rdsw").is_none() {
            log::debug!("BSIM4: rdsw not specified, using default {:.1} ohm*um", p.rdsw);
        }
