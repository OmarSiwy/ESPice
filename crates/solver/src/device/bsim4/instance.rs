...
pub struct Bsim4Instance {
    ...
    pub coxe: f64, // F/m^2
    pub coxp: f64,
    pub toxe: f64, // electrical oxide thickness (m) — used in narrow-width Vth term
    ...
    // ── bulk junction capacitances (area-based) ─────────────────────────
    pub cjd: f64,  // drain  bottom-junction cap (F) = cjd_per_area * AD
    pub cjs: f64,  // source bottom-junction cap (F) = cjs_per_area * AS
...
impl Bsim4Instance {
    ...
    pub fn from_model(model: &Bsim4Model, geom: &Bsim4Geometry, temp_k: f64) -> Self {
        ...
        .max(1.0e-1);
        let phi_default = 2.0 * vt_nom * (p.ndep / 1.45e10_f64.max(ni_t)).ln().abs();
        ...
        Self {
            ...
            // Junction caps: Cj_total = cj_per_area * Area.
            // If AD/AS not given, use default Weff * 0.
            cjd: p.cjd * geom.AD,
            cjs: p.cjs * geom.AS,
            pbd: p.pbd.max(0.01),
            pbs: p.pbs.max(0.01),
