...
// `incspice_device::eval::DeviceEval` infrastructure already understands.
//
...
use crate::device::eval::DeviceEval;

use super::eval::{Bsim4Eval, evaluate_dc};
...
pub fn stamp_bsim4(inst: &Bsim4Instance, voltages: &[f64]) -> DeviceEval {
    ...
    // Bulk junction capacitances: Cbd between D(0) and B(3), Cbs between S(2) and B(3).
    let cbd = e.cbd;
    let cbs = e.cbs;
    ...
    let C: SmallVec<[(u8, u8, f64); 16]> = smallvec![
        ...
        (0, 2, cds),
        (0, 3, cdb - cbd),
        // Source row (pin 2) — charge conservation + Cbs junction
        (2, 1, -(cgg + cdg)),
        (2, 0, -(cgd + cdd)),
        (2, 2, -(cgs + cds) + cbs),
        (2, 3, -(cgb + cdb) - cbs),
        // Bulk row (pin 3) — junction caps
        (3, 0, -cbd),
        (3, 2, -cbs),
        (3, 3, cbd + cbs),
    ];
