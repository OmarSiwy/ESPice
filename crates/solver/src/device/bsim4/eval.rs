...
pub struct Bsim4Eval {
    ...
    pub cbd: f64, // drain-bulk  junction cap (F)
    pub cbs: f64, // source-bulk junction cap (F)
...
fn clamp_vbs(vbs_in: f64, vbsc: f64) -> f64 {
    if vbs_in < vbsc { vbsc } else { vbs_in }
}
...
fn compute_vth(inst: &Bsim4Instance, vbs_eff: f64, vds: f64) -> (f64, f64, f64, f64) {
    ...
    // Narrow-width roll-off: (K3 + K3B*Vbs) * Toxe / (Weff + W0) * phi
    // Toxe factor (in metres) keeps the term dimensionally consistent with
    // BSIM3 and ensures the narrow-width correction is sub-volt for µm-scale
    // devices even with the default K3 = 80.
    let nw = (inst.k3 + inst.k3b * vbs_eff) * inst.toxe / (inst.weff + inst.w0).max(1.0e-9) * phi;
    ...
    let dvth_dvb = dbody_dvb + (inst.k3b * inst.toxe / (inst.weff + inst.w0).max(1.0e-9) * phi) - inst.etab * vds * dibl_exp;
...
fn compute_vdsat(inst: &Bsim4Instance, vgsteff: f64, vbs_eff: f64) -> (f64, f64, f64, f64) {
    let leff = inst.leff;
    ...
    let dvdsat_dvg = (esatl * denom - esatl * vgst2vtm * 1.0) / (denom * denom)
        + (esatl * vgst2vtm * (-inst.a0 * inst.ags * (1.0 - inst.keta * vbs_eff) * esatl))
            / (denom * denom);
    let dvdsat_dvb = -(esatl * vgst2vtm * (-abulk_dvb(inst, vgsteff) * esatl)) / (denom * denom);
    let dvdsat_dvd = 0.0;
...
pub fn evaluate_dc(inst: &Bsim4Instance, vd_t: f64, vg_t: f64, vs_t: f64, vb_t: f64) -> Bsim4Eval {
    ...
    // Step 4 — Vdsat.
    let (vdsat, dvdsat_dvg, _dvdsat_dvd, dvdsat_dvb) = compute_vdsat(inst, vgsteff, vbs_eff);
    ...
    let gds = ids * (dvdseff_dvd * inv_vdseff) + ids * (dvdsat_dvg * 0.0) + ids_core / va_clm;
    ...
    // ── Bulk junction capacitances ───────────────────────────────────────
    // BSIM4 area-based model: Cj = cj_per_area * Area * (1 - Vj/Pb)^(-mj)
    // Vdb = Vd - Vb = Vds - Vbs (in internal NMOS-equivalent frame).
    // Vsb = -Vbs.
    let fc_junc = 0.5_f64; // forward-bias coefficient
    let vdb = vds - vbs_eff;
    let vsb = -vbs_eff;

    let cbd = if inst.cjd > 0.0 {
        let v = -vdb; // junction voltage (body relative to drain, reverse = negative)
        let v_limit = fc_junc * inst.pbd;
        if v <= v_limit {
            inst.cjd * (1.0 - v / inst.pbd).powf(-inst.mjd)
        } else {
            let fc_term = (1.0 - fc_junc).powf(-(1.0 + inst.mjd));
            inst.cjd * fc_term
        }
    } else {
        0.0
    ...
    let cbs = if inst.cjs > 0.0 {
        let v = -vsb; // junction voltage (body relative to source)
        let v_limit = fc_junc * inst.pbs;
        if v <= v_limit {
            inst.cjs * (1.0 - v / inst.pbs).powf(-inst.mjs)
        } else {
            let fc_term = (1.0 - fc_junc).powf(-(1.0 + inst.mjs));
            inst.cjs * fc_term
        }
    } else {
        0.0
