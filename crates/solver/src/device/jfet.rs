...
fn ids_parker_skellern(
    ...
    let dvst_dvov =
        0.5 * (1.0 + vst_raw / (vst_raw * vst_raw + smooth_knee).sqrt()) * dvst_raw_dvov;
    ...
    // gm = d(Ids)/d(Vgs)
    let gm_core =
        beta / denom * (d_veff2_dvgs * clm * block - hfeta * veff * veff * clm * block / denom);
    let gm = gm_core;

    // gds = d(Ids)/d(Vds)
    let gds_core = beta / denom
        * (d_veff2_dvds * clm * block
    ...
    fn eval(&self, voltages: &[f64], params: &ParamMap) -> DeviceEval {
        ...
        let (id, dgm, gds) = if level == 2 {
            ...
            if vov <= 0.0 {
                ...
                // Triode region.
                let id =
                    beta * (2.0 * vov * vds - vds * vds) * (1.0 + lambda * vds) + GDS_MIN * vds;
...
mod tests {
    ...
    fn njfet_analytical_jacobian_vs_fd() {
        ...
        let anal_gds = eval0
            ...
            .unwrap_or(0.0);
        assert!(
            (fd_gds - anal_gds).abs() < 1e-6,
            "gds FD={fd_gds} anal={anal_gds}"
        );
    }
    ...
    fn jfet_vtotc_shifts_vto() {
        ...
        assert!(
            eval_tc.g[0].is_finite(),
            "Id with VTOTC must be finite: {}",
            eval_tc.g[0]
        );
    }
