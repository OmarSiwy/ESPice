...
use incspice_core::{BsourceExpr, DeviceKind};
use smallvec::{SmallVec, smallvec};

use crate::device::eval::{DeviceEval, DeviceModel};
#[allow(unused_imports)]
use crate::device::expr_ast::{EvalCtx, ExprAst};
...
impl DeviceModel for BsourceVModel {
    fn eval(&self, _voltages: &[f64], _params: &incspice_core::ParamMap) -> DeviceEval {
...
pub fn eval_bsource_v(
    ...
    branch_current: f64,
) -> Result<DeviceEval, incspice_core::SimError> {
    // f = expression value
...
mod tests {
    use super::*;
    use incspice_core::{BehavioralBinOp, BehavioralExpr, BsourceExpr};

    fn lit(v: f64) -> BehavioralExpr {
        BehavioralExpr::Lit(v)
    }
    fn nv(n: &str) -> BehavioralExpr {
        BehavioralExpr::NodeVoltage(n.into())
    }
    ...
    fn bsource_v_jacobian_finite_difference() {
        ...
        let expr =
            BehavioralExpr::BinOp(BehavioralBinOp::Pow, Box::new(nv("x")), Box::new(lit(2.0)));
        let bse = BsourceExpr::new(expr);
