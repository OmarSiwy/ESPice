...
impl DeviceModel for CSwitch {
    fn eval(&self, voltages: &[f64], params: &ParamMap) -> DeviceEval {
        ...
        // Use a sharpness factor large enough that tanh saturates to ±1 within
        // the hysteresis band regardless of the Gon/Goff ratio.  The residual
        // error from imperfect tanh saturation scales as Gon * exp(2*arg), so
        // for Gon/Goff ≈ 1e9 we need |arg| > ln(1e9) / 2 ≈ 10.4 at the edge
        // of the hysteresis band (|ic - it_eff| = ih_eff).  A factor of 20
        // gives |arg| ≥ 20 at the band edge, ensuring tanh saturates to
        // machine precision for any practical Gon/Goff ratio up to ~1e17.
        let arg = 20.0 * (ic - it_eff) / ih_eff;
        let th = arg.tanh();
        let g = 0.5 * (gon + goff) + 0.5 * (gon - goff) * th;
...
mod tests {
    ...
    fn switch_off_high_resistance() {
        ...
        assert!(
            (eval.g[0] - g_expected * 1.0).abs() < 1e-10,
            "g[0]={} expected≈{}",
            eval.g[0],
            g_expected
        );
    ...
    fn switch_on_low_resistance() {
        ...
        assert!(
            (eval.g[0] - g_expected * 5.0).abs() < 1e-10,
            "g[0]={} expected≈{}",
            eval.g[0],
            g_expected * 5.0
        );
    ...
    fn switch_jacobian_structure() {
        ...
        assert_eq!(eval.G[0].1, 0);
        assert_eq!(eval.G[1].0, 0);
        assert_eq!(eval.G[1].1, 1);
        assert_eq!(eval.G[2].0, 1);
        assert_eq!(eval.G[2].1, 0);
        assert_eq!(eval.G[3].0, 1);
        assert_eq!(eval.G[3].1, 1);
        ...
        assert_eq!(eval.G[4].0, 0);
        assert_eq!(eval.G[4].1, 2);
        assert_eq!(eval.G[5].0, 0);
        assert_eq!(eval.G[5].1, 3);
        assert_eq!(eval.G[6].0, 1);
        assert_eq!(eval.G[6].1, 2);
        assert_eq!(eval.G[7].0, 1);
        assert_eq!(eval.G[7].1, 3);
        ...
        assert!(
            cross < 1e-3,
            "cross-term far from threshold should be small, got {}",
            cross
        );
    }
    ...
    fn switch_smooth_at_threshold() {
        ...
        assert!(
            (eval.G[0].2 - g_mid).abs() < 1e-15,
            "at threshold G[0,0] should equal (Gon+Goff)/2 = {}, got {}",
            g_mid,
            eval.G[0].2
        );
        ...
        assert!(
            eval.G[4].2.abs() > 1e-3,
            "cross-term at threshold should be significant, got {}",
            eval.G[4].2
        );
    }
}
