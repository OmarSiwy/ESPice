...
fn least_squares_normal_equations(
    ...
    for j in 0..n_cols {
        for i in 0..=j {
            let dot: f64 = a_cols[i].iter().zip(a_cols[j].iter()).map(|(&ai, &aj)| ai * aj).sum();
            gram[i * n_cols + j] = dot;
            gram[j * n_cols + i] = dot;
        }
    }
...
impl AndersonAcceleration {
    ...
    pub fn step(&mut self, x_k: &[f64], f_k: &[f64]) -> Vec<f64> {
        ...
        if hist_len < 2 {
            return x_k.iter().zip(f_k.iter()).map(|(&x, &f)| x + self.beta * f).collect();
        }
...
mod tests {
    ...
    fn anderson_accelerates_cosine_fixedpoint() {
        ...
        assert!(aa_iters < 50, "AA should converge in fewer than 50 iterations, got {aa_iters}");
    }
    ...
    fn anderson_2d_contraction_converges() {
        ...
        assert!(
            (x[0] - x_sol).abs() < 1e-9,
            "x={} expected {x_sol}", x[0]
        );
        assert!(
            (x[1] - y_sol).abs() < 1e-9,
            "y={} expected {y_sol}", x[1]
        );
    }
