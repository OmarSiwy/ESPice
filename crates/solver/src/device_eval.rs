...
//!
//! The CPU path uses `incspice_device::bsim4::evaluate_dc` (the same
//! routine the per-device eval calls).  The GPU path delegates to
//! `incspice_compute::WgpuBackend` — currently this loads the
...
use crate::NrConfig;
#[cfg(feature = "gpu")]
use crate::compute::WgpuBackend;
use crate::device::{Bsim4Eval, Bsim4Instance, evaluate_dc};
...
#[cfg(not(feature = "gpu"))]
fn try_eval_bsim4_gpu(
    _instances: &[Bsim4Instance],
    _voltages: &[(f64, f64, f64, f64)],
) -> Option<Vec<Bsim4Eval>> {
    None
}
