...
//!
//! `OutputFormat` is the runtime dispatch tag used to pick a writer at the
//! command-line / config layer.  The numeric and string forms are stable so
//! they can be persisted in test configs and config files.
...
impl OutputFormat {
    /// Lowercase canonical name (`"rawbin"`, `"rawascii"`, `"hspice"`, `"touchstone"`, `"csv"`).
    ...
    /// Parse a canonical format name (case-insensitive). Accepts a few aliases.
...
mod tests {
    ...
    fn parse_canonical_names() {
        assert_eq!(
            OutputFormat::from_name("rawbin").unwrap(),
            OutputFormat::RawfileBinary
        );
        assert_eq!(
            OutputFormat::from_name("HSPICE").unwrap(),
            OutputFormat::HspicePost
        );
        assert_eq!(
            OutputFormat::from_name("touchstone").unwrap(),
            OutputFormat::Touchstone
        );
        assert_eq!(OutputFormat::from_name("csv").unwrap(), OutputFormat::Csv);
        assert!(OutputFormat::from_name("garbage").is_err());
    }

    #[test]
    fn parse_from_extension() {
        assert_eq!(
            OutputFormat::from_extension("raw").unwrap(),
            OutputFormat::RawfileBinary
        );
        assert_eq!(
            OutputFormat::from_extension(".tr0").unwrap(),
            OutputFormat::HspicePost
        );
        assert_eq!(
            OutputFormat::from_extension("s2p").unwrap(),
            OutputFormat::Touchstone
        );
        assert_eq!(
    ...
    #[test]
    fn round_trip_names_and_extensions() {
        for f in [
            OutputFormat::RawfileBinary,
            ...
            assert_eq!(OutputFormat::from_name(f.as_str()).unwrap(), f);
            // Default extensions parse back to the same family.
            let parsed = OutputFormat::from_extension(f.default_extension()).unwrap();
            // Touchstone defaults to .s2p which parses to Touchstone, etc.
            // RawfileAscii's default ext is also "raw" so it round-trips to RawfileBinary;
            // that's the documented "guess" behavior.
            if f == OutputFormat::RawfileAscii {
                assert_eq!(parsed, OutputFormat::RawfileBinary);
            } else {
                assert_eq!(parsed, f);
            }
        }
