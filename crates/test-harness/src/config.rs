...
impl ExternalSuite {
    /// Discover netlists by glob pattern, creating synthetic TestConfig entries.
...
mod tests {
    ...
    #[test]
    fn parse_test_toml() {
        let dir = std::env::temp_dir().join("pisim_test_config");
...
name = "rc_transient"
type = "regression"
tags = ["serial", "nightly", "tran"]
netlist = "circuit.sp"
golden = "golden.csv"
ignore = "requires transient integration"
...
"#).unwrap();
        ...
        assert_eq!(config.test.name, "rc_transient");
        assert_eq!(config.test.test_type, "regression");
        assert!(config.is_ignored());
        assert!(config.matches_tags(&["serial".into()]));
        assert!(!config.matches_tags(&["weekly".into()]));
        assert_eq!(config.netlist_path(), dir.join("circuit.sp"));

        std::fs::remove_dir_all(&dir).ok();
    }

    #[test]
    fn parse_test_toml_with_reference() {
        let dir = std::env::temp_dir().join("pisim_test_config_ref");
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("test.toml");
        let mut f = std::fs::File::create(&path).unwrap();
        write!(f, r#"
...
"#).unwrap();
        ...
        let config = TestConfig::from_file(&path).unwrap();
        assert_eq!(config.reference.simulator, "ngspice");

        std::fs::remove_dir_all(&dir).ok();
    }

    #[test]
    fn parse_test_toml_reference_defaults() {
        let dir = std::env::temp_dir().join("pisim_test_config_ref_default");
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("test.toml");
        let mut f = std::fs::File::create(&path).unwrap();
        write!(f, r#"
...
"#).unwrap();
    ...
    #[test]
    fn external_suite_discover() {
        let dir = std::env::temp_dir().join("pisim_test_ext_suite");
        let sub = dir.join("tests").join("sub");
        std::fs::create_dir_all(&sub).unwrap();

        std::fs::write(sub.join("test1.cir"), "* test1\nV1 1 0 DC 5\nR1 1 0 1k\n.OP\n.END\n").unwrap();
        std::fs::write(sub.join("test2.cir"), "* test2\nV1 1 0 DC 3\nR1 1 0 1k\n.OP\n.END\n").unwrap();
        std::fs::write(sub.join("readme.txt"), "not a netlist").unwrap();

        let suite = ExternalSuite {
            name: "test_suite".to_string(),
            root: dir.join("tests"),
            netlist_glob: "**/*.cir".to_string(),
            default_tags: vec!["external".to_string(), "nightly".to_string()],
        };

        let configs = suite.discover();
        assert_eq!(configs.len(), 2);

        for cfg in &configs {
            assert!(cfg.test.tags.contains(&"external".to_string()));
            assert!(cfg.test.netlist.ends_with(".cir"));
            assert_eq!(cfg.test.test_type, "external");
        }
