...
fn bipartite_matching(a: &CscMatrix, n: usize) -> (Vec<usize>, Vec<usize>) {
    ...
    for col_start in 0..n {
        ...
        augment(a, col_start, stamp, &mut col_match, &mut row_match, &mut visited);
    }
...
mod tests {
    ...
    fn duff_reid_6x6() {
        ...
        assert!(btf.block_count >= 2, "expected at least 2 blocks, got {}", btf.block_count);
        assert!(!is_btf_trivial(&btf), "should not be trivial for this matrix");
        check_block_upper_triangular(&a, &btf);
    }
