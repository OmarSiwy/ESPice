...
impl CscMatrix {
    ...
    pub fn from_triplet(triplet: &TripletMatrix) -> Self {
        ...
        // SoA fast path: pull each field as a contiguous slice from the
        // triplet matrix.  Each pass below touches only the field(s) it
        // actually needs, which is the whole point of the SoA layout.
        let trip_rows = triplet.row_idx();
        let trip_cols = triplet.col_idx();
        let trip_vals = triplet.vals();

        // --- Step 1: count entries per column (cols-only sweep) ---
        ...
        let mut cursor = col_ptr[..ncols].to_vec(); // write cursor per column
        ...
        // --- Step 4: sort each column by row index, summing duplicates ---
        for j in 0..ncols {
            let start = col_ptr[j];
            let end = col_ptr[j + 1];
            if start == end {
            ...
            // Sort by row index (insertion sort — columns are typically short)
    ...
    pub fn identity(n: usize) -> Self {
        ...
        Self { nrows: n, ncols: n, col_ptr, row_idx, values }
    }
...
impl CscMatrix {
    ...
    pub fn transpose(&self) -> CscMatrix {
        // A^T in CSC is equivalent to A in CSR.  Build by treating rows of A
        // as columns of A^T.
        let nnz = self.nnz();
        let nt_rows = self.ncols; // rows of A^T = cols of A
        let nt_cols = self.nrows; // cols of A^T = rows of A

        // Count entries per column of A^T (= per row of A)
        ...
        for j in 0..self.ncols {
            ...
            for idx in start..end {
                ...
                let pos = cursor[r];
                row_idx[pos] = j; // column j of A becomes row j of A^T
                values[pos] = self.values[idx];
                cursor[r] += 1;
            }
        }

        // Sort row indices within each column (they may not be in order)
        for j in 0..nt_cols {
            let s = col_ptr[j];
            let e = col_ptr[j + 1];
    ...
    /// Sparse matrix addition: `C = A + B`.  Both must have the same
    /// dimensions.
