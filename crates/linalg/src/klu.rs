...
impl KluSolver {
    ...
    pub fn factorize(matrix: &CscMatrix) -> Result<Self, SimError> {
        #[cfg(feature = "klu")]
        {
            ...
            let col_ptr_i32: Vec<i32> =
                matrix.col_ptr().iter().map(|&x| x as i32).collect();
            let row_idx_i32: Vec<i32> =
                matrix.row_idx().iter().map(|&x| x as i32).collect();
        ...
        #[cfg(feature = "klu")]
        {
            ...
            let ok = unsafe {
                ffi::klu_solve(
                    ...
                    1,
                    x.as_mut_ptr(),
                    self.inner.common.as_ref() as *const ffi::KluCommon
                        as *mut ffi::KluCommon,
                )
            };
            if ok == 0 {
