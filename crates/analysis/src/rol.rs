...
use incspice_core::{Circuit, DeviceKind, SimError};

// Re-export RolConfig from incspice_core so callers can use it via this crate.
pub use incspice_core::RolConfig;
...
pub fn run_rol(
    dc_result: &DcOpResultVec,
    ckt: &Circuit,
    cfg: &RolConfig,
) -> Result<RolResult, SimError> {
    if cfg.lifetime <= 0.0 {
        return Err(SimError::Analysis("ROL: lifetime must be positive".into()));
    }
    if cfg.temp <= 0.0 {
...
mod tests {
    ...
    fn rol_em_basic() {
        ...
        let dc_out = run_dc_op_internal(&ckt, &reg).unwrap();

        let cfg = RolConfig::default();
        let result = run_rol(&dc_out.result, &ckt, &cfg).unwrap();

        // Both R1 and R2 should have EM entries.
        let em_entries: Vec<_> = result.devices.iter().filter_map(|d| d.em_mttf).collect();
    ...
    fn rol_em_temperature_dependence() {
        ...
        let cfg_cool = RolConfig {
            temp: 300.0,
            ..RolConfig::default()
        };
        let cfg_hot = RolConfig {
            temp: 400.0,
            ..RolConfig::default()
        };
