...
mod tests {
    ...
    fn lhs_stratification_property() {
        ...
        for dim in 0..d {
            ...
            for s in 0..n {
                ...
                assert!(
                    !stratum_hit[stratum],
                    "dim={dim}: stratum {stratum} hit twice"
                );
                stratum_hit[stratum] = true;
            }
            assert!(
                stratum_hit.iter().all(|&h| h),
    ...
    #[test]
    fn sampling_config_default_is_lhs() {
        let cfg = SamplingConfig::default();
        assert_eq!(cfg.strategy, SamplingStrategy::LatinHypercube);
        assert_eq!(cfg.nsamples, 100);
        assert!(!cfg.parallel);
    }

    #[test]
    fn lhs_unit_cube_size() {
        let mat = lhs_unit_cube(10, 3, 0);
        assert_eq!(mat.len(), 30); // n * d
    }

    #[test]
    fn mc_unit_cube_size() {
        let mat = mc_unit_cube(7, 4, 42);
        assert_eq!(mat.len(), 28);
    }

    #[test]
    fn lhs_different_seeds_give_different_results() {
        let a = lhs_unit_cube(10, 2, 111);
        let b = lhs_unit_cube(10, 2, 222);
        let same = a.iter().zip(b.iter()).all(|(x, y)| (x - y).abs() < 1e-15);
        assert!(!same, "different seeds should produce different matrices");
    }

    #[test]
    fn mc_unit_cube_values_in_unit_interval_detailed() {
        let mat = mc_unit_cube(50, 4, 999);
        for (i, &v) in mat.iter().enumerate() {
            assert!(v >= 0.0 && v < 1.0, "mc[{i}]={v} out of [0,1)");
        }
    }

    #[test]
    fn probit_at_0_5_is_zero() {
        assert!(probit(0.5).abs() < 0.01);
    }

    #[test]
    fn probit_symmetric_around_half() {
        for p in &[0.1, 0.2, 0.3, 0.4] {
            let z = probit(*p);
            let z_mirror = probit(1.0 - p);
            assert!((z + z_mirror).abs() < 0.02, "probit({p})+probit({})={}", 1.0-p, z+z_mirror);
        }
    }
    ...
    fn percentile_single_element() {
        ...
        assert!((percentile(&data, 0.0) - 42.0).abs() < 1e-15);
        assert!((percentile(&data, 100.0) - 42.0).abs() < 1e-15);
        assert!((percentile(&data, 50.0) - 42.0).abs() < 1e-15);
    }

    #[test]
    fn percentile_two_elements() {
        let data = vec![0.0_f64, 100.0];
        assert!((percentile(&data, 0.0) - 0.0).abs() < 1e-10);
        assert!((percentile(&data, 100.0) - 100.0).abs() < 1e-10);
        assert!((percentile(&data, 50.0) - 50.0).abs() < 0.01);
    }

    #[test]
    fn summary_single_value() {
        let vals = vec![7.0_f64];
        let s = SamplingSummary::from_values("x", &vals);
        assert_eq!(s.n, 1);
        assert!((s.mean - 7.0).abs() < 1e-15);
        assert!((s.std - 0.0).abs() < 1e-15, "single-element std should be 0");
    }

    #[test]
    fn summary_filters_nan() {
        let vals = vec![1.0_f64, f64::NAN, 3.0, f64::NAN, 5.0];
        let s = SamplingSummary::from_values("x", &vals);
        assert_eq!(s.n, 3);
        assert!((s.mean - 3.0).abs() < 1e-10);
    }
    ...
    #[test]
    fn lhs_1d_stratification() {
        // With n=5, d=1 each value in mat must fall in a unique stratum [k/5, (k+1)/5).
        let n = 5;
        let mat = lhs_unit_cube(n, 1, 0xDEAD);
        let mut strata = vec![false; n];
        for &v in &mat {
            let stratum = (v * n as f64).floor() as usize;
            assert!(!strata[stratum], "stratum {stratum} hit twice");
            strata[stratum] = true;
        }
        assert!(strata.iter().all(|&h| h), "not all strata covered");
    }

    #[test]
    fn mc_strategy_in_run_sampling() {
        let base = build_divider();
        let reg = DeviceRegistry::new_default();
        let stat = StatExpr {
            kind: StatKind::Limit,
        ...
        let cfg = SamplingConfig {
            ...
            seed: 0x1234,
            parallel: false,
            strategy: SamplingStrategy::MonteCarlo,
        };
        let results = run_sampling(
            &base, &overrides, vec!["v_out".into()], &cfg,
            |ckt, _idx| {
                let out = crate::dc_op::run_dc_op(ckt, &reg)
                    .map_err(|e| SimError::Analysis(format!("{e}")))?;
                let v = out.result.node_voltages.iter()
                    .find(|(n, _)| n == "2").map(|(_, v)| *v).unwrap_or(f64::NAN);
                Ok(vec![v])
            },
        ).unwrap();
        assert_eq!(results.nsamples, 11); // 10 + 1 nominal
    }
}
