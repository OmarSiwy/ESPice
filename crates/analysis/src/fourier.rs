...
impl FourierConfig {
    pub fn new(freq: f64, node_index: usize) -> Self {
        Self { freq, node_index, num_harmonics: 9 }
    }
...
impl FourierSignal {
    ...
    pub fn from_transient(result: &TransientResult, node_index: usize) -> Self {
        let values: Vec<f64> = (0..result.num_steps())
            ...
            .collect();
        Self { times: result.times.clone(), values }
    }
}
...
pub fn run_fourier(signal: &FourierSignal, config: &FourierConfig) -> Result<FourierResult, SimError> {
    ...
    if t_start < signal.times[0] {
        return Err(SimError::Analysis(
            format!("fourier: waveform too short for one full period at {:.3e} Hz (need >= {:.3e} s, have {:.3e} s)",
                config.freq, period, t_end - signal.times[0])
        ));
    }
    ...
    Ok(FourierResult { dc, harmonics, thd_pct })
}
...
pub fn run_fft(_signal: &FourierSignal, _config: &FourierConfig) -> Result<FourierResult, SimError> {
...
mod tests {
    ...
    fn sine_signal(freq: f64, amplitude: f64, n: usize) -> FourierSignal {
        ...
        let values: Vec<f64> = times.iter().map(|&t| amplitude * (2.0 * std::f64::consts::PI * freq * t).sin()).collect();
    ...
    fn dc_only_signal() {
        ...
        for h in &result.harmonics {
            assert!(h.magnitude < 1e-6, "harmonic {} magnitude = {}", h.index, h.magnitude);
        }
    }
    ...
    fn pure_sine_fundamental() {
        ...
        assert!((fund.magnitude - amp).abs() < 0.01, "fundamental magnitude = {}", fund.magnitude);
        for h in &result.harmonics[1..] {
            assert!(h.magnitude < 0.05 * amp, "harmonic {} = {}", h.index, h.magnitude);
        }
    ...
    fn run_fft_returns_unsupported() {
        ...
        if let Err(SimError::Analysis(msg)) = result {
            assert!(msg.contains(".FFT"), "error message should mention .FFT: {msg}");
        } else {
