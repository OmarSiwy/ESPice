...
//! analysis execution to the existing `incspice_analysis` functions.
//!
...
use crate::result::{DcOpResultVec, ResultData, TransientResult};
use crate::transient::{TransientConfig, run_transient};
...
pub struct SimResults {
    ...
    pub dc_op: Option<DcOpResultVec>,
...
impl<'a> ControlInterpreter<'a> {
    ...
    fn exec_op(&mut self) -> Result<(), SimError> {
        let out = run_dc_op_internal(self.circuit, self.registry)?;
        self.results.node_names = out
            .result
            .node_voltages
            .iter()
            .map(|(n, _)| n.clone())
            .collect();
        self.results.dc_op = Some(DcOpResultVec {
    ...
    fn exec_tran(&mut self, rest: &str) -> Result<(), SimError> {
        ...
        if parts.len() < 2 {
            return Err(SimError::Parse("control: tran requires tstep tstop".into()));
        }
        ...
        if !rows.is_empty() {
            ...
            for i in 0..n_rows {
                let line: Vec<String> = rows
                    .iter()
                    .map(|col| col.get(i).map(|v| v.to_string()).unwrap_or_default())
                    .collect();
    ...
    fn eval_condition(&self, cond: &str) -> Result<bool, SimError> {
        ...
        for op in &[">=", "<=", "!=", ">", "<", "=="] {
            if let Some(pos) = cond.find(op) {
                ...
                let l = self.eval_expr(lhs)?.as_number().ok_or_else(|| {
                    SimError::Parse(format!("control: non-numeric LHS in condition '{cond}'"))
                })?;
                let r = self.eval_expr(rhs)?.as_number().ok_or_else(|| {
                    SimError::Parse(format!("control: non-numeric RHS in condition '{cond}'"))
                })?;
                return Ok(match *op {
                    ">=" => l >= r,
                    "<=" => l <= r,
...
pub fn collect_block_body(
    lines: &[String],
    start: usize,
) -> Result<(Vec<String>, usize), SimError> {
    ...
    Err(SimError::Parse(
        "control: unterminated block (missing 'end')".into(),
    ))
}
...
mod tests {
    ...
    fn parse_si_value_basic() {
        let eps = 1e-14;
        assert!((parse_si_value("1k").unwrap() - 1e3).abs() < eps * 1e3);
        assert!((parse_si_value("100n").unwrap() - 100e-9).abs() < eps * 100e-9);
        assert!((parse_si_value("2.2u").unwrap() - 2.2e-6).abs() < eps * 2.2e-6);
        assert!((parse_si_value("1meg").unwrap() - 1e6).abs() < eps * 1e6);
        assert!((parse_si_value("3.14").unwrap() - 3.14).abs() < eps * 3.14);
    ...
    fn let_and_print() {
        ...
        let lines = vec!["let x = 42".to_string(), "print x".to_string()];
