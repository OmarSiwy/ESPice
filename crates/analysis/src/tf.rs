...
fn run_tf_inner(
    ...
    let input_dev = circuit.find_device(&cfg.input_source).ok_or_else(|| {
        SimError::Analysis(format!("TF: input source '{}' not found", cfg.input_source))
    })?;
...
fn resolve_node_row(
    ...
    if row >= num_nodes {
        return Err(SimError::Analysis(format!(
            "TF: node '{name}' row {row} out of range"
        )));
    }
    Ok(Some(row))
}
