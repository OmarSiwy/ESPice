...
pub(crate) fn run_fft(signal: &FourierSignal, cfg: &FftConfig) -> Result<FftResult, SimError> {
    if signal.times.len() < 2 {
        return Err(SimError::Analysis("FFT: need at least two samples".into()));
    }
    let dt_avg =
        (signal.times.last().unwrap() - signal.times[0]) / (signal.times.len() as f64 - 1.0);
    if dt_avg <= 0.0 {
