...
mod tests {
    ...
    #[test]
    fn schedule_one_event_len_is_one() {
        let mut q = EventQueue::with_capacity(4);
        q.schedule(1.0, DigNodeIdx::new(0), DigState::ONE);
        assert_eq!(q.len(), 1);
        assert!(!q.is_empty());
    ...
    #[test]
    fn pop_due_clears_output_first() {
        let mut q = EventQueue::with_capacity(4);
        q.schedule(1.0, DigNodeIdx::new(0), DigState::ONE);
        let mut out = vec![Event { time: 99.0, node: DigNodeIdx::new(99), value: DigState::X }];
        q.pop_due(1.0, &mut out);
        // Output should be cleared and then contain the one popped event.
        assert_eq!(out.len(), 1);
        assert!((out[0].time - 1.0).abs() < 1e-15);
    }

    #[test]
    fn pop_due_nothing_due() {
        let mut q = EventQueue::with_capacity(4);
        q.schedule(5.0, DigNodeIdx::new(0), DigState::ONE);
        let mut out = Vec::new();
        q.pop_due(1.0, &mut out);
        assert!(out.is_empty());
        assert_eq!(q.len(), 1, "event should still be in queue");
    }

    #[test]
    fn rollback_all_future_leaves_empty() {
        let mut q = EventQueue::with_capacity(8);
        q.schedule(3.0, DigNodeIdx::new(0), DigState::ONE);
        q.schedule(4.0, DigNodeIdx::new(1), DigState::ZERO);
        q.rollback(0.0); // rollback before all events
        assert!(q.is_empty());
    }

    #[test]
    fn rollback_keeps_past_events() {
        let mut q = EventQueue::with_capacity(8);
        q.schedule(1.0, DigNodeIdx::new(0), DigState::ONE);
        q.schedule(2.0, DigNodeIdx::new(0), DigState::ZERO);
        q.schedule(5.0, DigNodeIdx::new(0), DigState::ONE);
        q.rollback(2.0);
        assert_eq!(q.len(), 2); // t=1 and t=2 kept; t=5 removed
    }
    ...
    fn clear_makes_empty() {
        let mut q = EventQueue::with_capacity(8);
        q.schedule(1.0, DigNodeIdx::new(0), DigState::ONE);
        q.schedule(2.0, DigNodeIdx::new(1), DigState::ZERO);
        q.clear();
        assert!(q.is_empty());
        assert_eq!(q.next_time(), None);
    }
    ...
    #[test]
    fn schedule_after_clear_reuses_slots() {
        let mut q = EventQueue::with_capacity(4);
        q.schedule(1.0, DigNodeIdx::new(0), DigState::ONE);
        let mut out = Vec::new();
        q.pop_due(1.0, &mut out);
        // Now re-schedule in the cleared slot
        q.schedule(2.0, DigNodeIdx::new(1), DigState::ZERO);
        assert_eq!(q.len(), 1);
    ...
    #[test]
    fn event_values_preserved_through_queue() {
        let mut q = EventQueue::with_capacity(8);
        q.schedule(1.0, DigNodeIdx::new(42), DigState::X);
        let mut out = Vec::new();
        q.pop_due(1.0, &mut out);
        assert_eq!(out[0].node, DigNodeIdx::new(42));
        assert_eq!(out[0].value, DigState::X);
    }

    #[test]
    fn min_heap_property_maintained() {
        let mut q = EventQueue::with_capacity(16);
        // Insert in reverse order
        for i in (0..10u32).rev() {
            q.schedule(i as f64, DigNodeIdx::new(i), DigState::ONE);
        }
        // Pop one by one; times should be non-decreasing
        let mut last_t = -1.0_f64;
        let mut out = Vec::new();
        for _ in 0..10 {
            let next_t = q.next_time().unwrap();
            assert!(next_t >= last_t, "min-heap violated: {next_t} < {last_t}");
            q.pop_due(next_t, &mut out);
            last_t = out[0].time;
        }
    }
}
