...
impl DigState {
    ...
    pub const fn is_strong(self) -> bool {
        (self.0 >> STRENGTH_SHIFT) == Strength::Strong as u8 && (self.0 & LEVEL_MASK) != LEVEL_Z
    }
...
mod tests {
    ...
    #[test]
    fn dig_state_zero_constant_is_zero() {
        assert!(DigState::ZERO.is_zero());
        assert!(!DigState::ZERO.is_one());
        assert!(!DigState::ZERO.is_x());
        assert!(!DigState::ZERO.is_z());
    }

    #[test]
    fn dig_state_one_constant_is_one() {
        assert!(DigState::ONE.is_one());
        assert!(!DigState::ONE.is_zero());
        assert!(!DigState::ONE.is_x());
        assert!(!DigState::ONE.is_z());
    }

    #[test]
    fn dig_state_x_constant_is_x() {
        assert!(DigState::X.is_x());
        assert!(!DigState::X.is_zero());
        assert!(!DigState::X.is_one());
        assert!(!DigState::X.is_z());
    }

    #[test]
    fn dig_state_z_constant_is_z() {
        assert!(DigState::Z.is_z());
        assert!(!DigState::Z.is_zero());
        assert!(!DigState::Z.is_one());
        assert!(!DigState::Z.is_x());
    }
    ...
    #[test]
    fn z_has_hi_z_strength() {
        assert_eq!(DigState::Z.strength(), Strength::HiZ);
    }

    #[test]
    fn one_zero_strong_by_default() {
        assert_eq!(DigState::ONE.strength(), Strength::Strong);
        assert_eq!(DigState::ZERO.strength(), Strength::Strong);
    }

    #[test]
    fn x_is_strong_by_default() {
        assert_eq!(DigState::X.strength(), Strength::Strong);
        assert!(DigState::X.is_strong());
    }

    #[test]
    fn zero_with_resistive_strength() {
        let s = DigState::zero(Strength::Resistive);
        assert!(s.is_zero());
    ...
    #[test]
    fn one_with_weak_strength() {
        let s = DigState::one(Strength::Weak);
        assert!(s.is_one());
        assert_eq!(s.strength(), Strength::Weak);
    }

    #[test]
    fn x_with_resistive_strength() {
        let s = DigState::x(Strength::Resistive);
        assert!(s.is_x());
        assert_eq!(s.strength(), Strength::Resistive);
    }
    ...
    #[test]
    fn default_is_z() {
        assert_eq!(DigState::default(), DigState::Z);
    }

    #[test]
    fn display_format() {
        assert_eq!(format!("{}", DigState::ZERO), "0");
        assert_eq!(format!("{}", DigState::ONE), "1");
        assert_eq!(format!("{}", DigState::X), "X");
        assert_eq!(format!("{}", DigState::Z), "Z");
    }

    #[test]
    fn resolve_two_z_gives_z() {
        assert_eq!(DigState::resolve(DigState::Z, DigState::Z), DigState::Z);
    }

    #[test]
    fn resolve_resistive_vs_strong_strong_wins() {
        let resistive0 = DigState::zero(Strength::Resistive);
        let strong1 = DigState::ONE;
        // Strong wins over Resistive regardless of level
        let resolved = DigState::resolve(resistive0, strong1);
        assert_eq!(resolved, strong1);
    }

    #[test]
    fn resolve_weak_conflict_is_x_at_weak_strength() {
        let weak0 = DigState::zero(Strength::Weak);
        let weak1 = DigState::one(Strength::Weak);
        let r = DigState::resolve(weak0, weak1);
        assert!(r.is_x());
    ...
    fn strength_ordering() {
        assert!((Strength::HiZ as u8) < (Strength::Resistive as u8));
        assert!((Strength::Resistive as u8) < (Strength::Weak as u8));
        assert!((Strength::Weak as u8) < (Strength::Strong as u8));
    }
}
