//! `cli::io` — simulator output format writers.
//!
//! Consolidates every disk format the simulator can emit so that:
//!
...
//! | [`sinks`]        | Concrete `StreamingSink` impls (Rawfile, Csv, Channel)   |
...
pub use print_select::{PrintColumn, PrintSelectError, PrintSpec, resolve as resolve_print_specs};
...
pub use sinks::{ChannelSink, CsvSink, RawfileSink, spawn_streaming_pipeline};
