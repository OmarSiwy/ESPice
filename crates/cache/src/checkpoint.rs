...
//! ## BDF ring buffer
//!
//! BDF history is maintained as a constant-memory ring buffer in the solver;
//! the `TransientArena` stores only periodic checkpoints, not every timestep.
//! This keeps memory usage O(checkpoints) rather than O(timestep count).
//!
//! ## .MEAS selective buffering
//!
//! Only signals referenced by `.MEAS` statements need to be buffered in RAM.
//! All other signals stream directly to disk via the `StreamingSink` pipeline.
...
impl TransientArena {
    ...
    pub fn nearest_before(&self, t: f64) -> Option<CheckpointIdx> {
        ...
        let pos = self.times.partition_point(|&ts| ts <= t);
        if pos == 0 {
            None
        } else {
...
mod tests {
    ...
    #[test]
    fn get_owned_round_trip() {
        let mut arena = TransientArena::new();
        let idx = arena.push(3.14, &[1.0, 2.0, 3.0], &[0.1, 0.2], &[(1.0, 7)]);
        let snap = arena.get_owned(idx).unwrap();
        assert_eq!(snap.time, 3.14);
        assert_eq!(snap.state, vec![1.0, 2.0, 3.0]);
        assert_eq!(snap.charge_hist, vec![0.1, 0.2]);
        assert_eq!(snap.events, vec![(1.0, 7)]);
    }
}
