...
///
/// ```ignore
/// use incspice_core::param::{CompiledParams, ParamMap};
///
/// #[derive(Clone, Copy)]
/// struct MosfetParams {
///     kp: f64,
...
///
/// ```ignore
/// use incspice_core::param::{compiled_get, ParamMap};
...
mod tests {
    ...
    #[test]
    fn param_map_with_capacity_is_empty() {
        let pm = ParamMap::with_capacity(16);
        assert!(pm.is_empty());
    }

    #[test]
    fn param_map_contains_after_set() {
        let mut pm = ParamMap::new();
        pm.set("vth", 0.7);
        assert!(pm.contains("vth"));
        assert!(pm.contains("VTH"), "contains must be case-insensitive");
        assert!(!pm.contains("kp"));
    }

    #[test]
    fn param_map_overwrite_value() {
        let mut pm = ParamMap::new();
        pm.set("r", 1e3);
        pm.set("r", 2e3);
        assert_eq!(pm.get("r"), Some(2e3));
        assert_eq!(pm.len(), 1, "overwrite must not add a duplicate key");
    }

    #[test]
    fn param_map_remove_absent_returns_none() {
        let mut pm = ParamMap::new();
        assert_eq!(pm.remove("nothing"), None);
    }

    #[test]
    fn param_map_iter_yields_all_entries() {
        let mut pm = ParamMap::new();
        pm.set("a", 1.0);
        pm.set("b", 2.0);
        pm.set("c", 3.0);
        let mut keys: Vec<&str> = pm.iter().map(|(k, _)| k).collect();
        keys.sort_unstable();
        assert_eq!(keys, ["a", "b", "c"]);
    }

    #[test]
    fn param_map_get_or_returns_set_value() {
        let mut pm = ParamMap::new();
        pm.set("gmin", 1e-12);
        assert_eq!(pm.get_or("gmin", 1e-9), 1e-12);
    }

    #[test]
    fn param_map_get_or_returns_default_when_missing() {
        let pm = ParamMap::new();
        assert_eq!(pm.get_or("xyz", -1.0), -1.0);
    }

    #[test]
    fn param_key_stores_lowercase() {
        let k = ParamKey::new("LAMBDA");
        assert_eq!(k.as_str(), "lambda");
    }

    #[test]
    fn param_key_from_str() {
        let k: ParamKey = "RES".into();
        assert_eq!(k.as_str(), "res");
    }

    #[test]
    fn param_key_display() {
        let k = ParamKey::new("tox");
        assert_eq!(format!("{k}"), "tox");
    }

    #[test]
    fn param_map_long_key_fallback() {
        // Keys longer than 32 bytes fall back to heap allocation.
        let long_key = "a_very_long_parameter_name_exceeding_32_bytes_total";
        assert!(long_key.len() > 32);
        let mut pm = ParamMap::new();
        pm.set(long_key, 99.9);
        assert_eq!(pm.get(long_key), Some(99.9));
        assert!(pm.contains(long_key));
        assert_eq!(pm.remove(long_key), Some(99.9));
    }
}
