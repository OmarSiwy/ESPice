...
impl Waveform {
    ...
    pub fn evaluate_at(&self, t: f64) -> f64 {
        match self {
            ...
            //   [td+tr+pw+tf, per) → v1
            // ----------------------------------------------------------------
            &Self::Pulse { v1, v2, td, tr, tf, pw, per } => {
                if t < td {
                    return v1;
                }
            ...
            // ----------------------------------------------------------------
            &Self::Sin { vo, va, freq, td, theta } => {
                if t < td {
                    vo
                } else {
                    let t_eff = t - td;
                    let angle = core::f64::consts::TAU * freq * t_eff;
                    let damping = if theta == 0.0 { 1.0 } else { (-theta * t_eff).exp() };
            ...
            // ----------------------------------------------------------------
            &Self::Trrandom { kind, tstep, td, param, mean } => {
                if t < td || tstep <= 0.0 {
                    return mean;
                }
    ...
    pub fn load_pwl_csv(path: &str) -> Vec<(f64, f64)> {
        ...
        content
            ...
            .filter_map(|line| {
                ...
                let mut parts = line.split(|c: char| c == ',' || c.is_ascii_whitespace())
                    .filter(|s| !s.is_empty());
...
mod tests {
    ...
    fn pulse_before_delay() {
        ...
        let w = Waveform::Pulse {
            v1: 0.0, v2: 5.0,
            td: 1e-6, tr: 100e-9, tf: 100e-9, pw: 500e-9, per: 1e-6,
        };
        assert!((w.evaluate_at(0.0)).abs() < 1e-15, "at t=0 before delay");
        assert!((w.evaluate_at(500e-9)).abs() < 1e-15, "at t=500n before delay");
    }
    ...
    fn pulse_at_top() {
        ...
        let w = Waveform::Pulse {
            v1: 0.0, v2: 5.0,
            td: 1e-6, tr: 100e-9, tf: 100e-9, pw: 500e-9, per: 2e-6,
        };
        let t_top = 1e-6 + 100e-9 + 50e-9; // td + tr + half_pw
        assert!((w.evaluate_at(t_top) - 5.0).abs() < 1e-10, "should be v2 at flat top");
    }
    ...
    fn pulse_midpoint_of_rise() {
        ...
        assert!((v - 2.5).abs() < 1e-10, "midpoint of rise should be 2.5 V, got {v}");
    }
    ...
    fn pulse_midpoint_of_fall() {
        ...
        assert!((v - 2.5).abs() < 1e-10, "midpoint of fall should be 2.5 V, got {v}");
    }
    ...
    fn pulse_back_to_bottom() {
        ...
        assert!(v.abs() < 1e-10, "flat bottom of 2nd period should be 0 V, got {v}");
    }
    ...
    fn pulse_periodic_top_second_cycle() {
        ...
        assert!((v - 5.0).abs() < 1e-10, "2nd cycle flat top should be 5V, got {v}");
    }
    ...
    fn sin_at_zero_time() {
        ...
        // sin(0) = 0, so result = 0.
        let w = Waveform::Sin { vo: 0.0, va: 1.0, freq: 1e3, td: 0.0, theta: 0.0 };
    ...
    fn sin_at_quarter_period() {
        ...
        let freq = 1e3;
        let w = Waveform::Sin { vo: 0.0, va: 1.0, freq, td: 0.0, theta: 0.0 };
        let t_quarter = 1.0 / (4.0 * freq);
        let v = w.evaluate_at(t_quarter);
        assert!((v - 1.0).abs() < 1e-10, "sin at quarter period should be 1.0, got {v}");
    }

    #[test]
    fn sin_dc_offset() {
        // vo=2, va=1, at t=0 the result should be vo.
        let w = Waveform::Sin { vo: 2.0, va: 1.0, freq: 1e3, td: 0.0, theta: 0.0 };
        assert!((w.evaluate_at(0.0) - 2.0).abs() < 1e-15);
    }

    #[test]
    fn sin_before_delay_returns_vo() {
        let w = Waveform::Sin { vo: 3.0, va: 5.0, freq: 1e6, td: 1e-6, theta: 0.0 };
    ...
    fn pwl_exact_knot() {
        ...
        assert!((w.evaluate_at(1.0) - 5.0).abs() < 1e-10, "value at knot t=1");
    }
    ...
    fn exp_before_rise_delay() {
        let w = Waveform::Exp { v1: 0.0, v2: 1.0, td1: 1e-6, tau1: 0.5e-6, td2: 2e-6, tau2: 0.5e-6 };
    ...
    fn exp_asymptotes_to_v2() {
        ...
        let w = Waveform::Exp { v1: 0.0, v2: 5.0, td1: 0.0, tau1: 1e-6, td2: 100e-6, tau2: 1e-6 };
        let v = w.evaluate_at(10e-6); // 10 tau1 after td1, before td2
        assert!((v - 5.0).abs() < 0.001, "should be near v2=5 at 10*tau1, got {v}");
    }
    ...
    fn sffm_at_zero() {
        ...
        let w = Waveform::Sffm { vo: 1.0, va: 2.0, fc: 1e6, mdi: 5.0, fs: 1e3 };
    ...
    fn from_params_pulse_at_t0_before_delay() {
        ...
        let w = Waveform::from_params(&p);
        assert!(w.evaluate_at(0.0).abs() < 1e-15, "PULSE before td should be v1=0");
    }
    ...
    fn from_params_pwl() {
        ...
        assert!((v - 2.5).abs() < 1e-10, "PWL midpoint should be 2.5V, got {v}");
    }
    ...
    fn am_before_delay_returns_zero() {
        let w = Waveform::Am { vo: 1.0, va: 1.0, fc: 1e3, freq: 1e6, td: 1e-6 };
        assert!(w.evaluate_at(0.5e-6).abs() < 1e-15, "AM before td must be 0");
    }
    ...
    fn am_at_carrier_zero_crossing() {
        ...
        let freq = 1e6;
        let w = Waveform::Am { vo: 0.5, va: 1.0, fc: 1e3, freq, td: 0.0 };
        let t_full = 1.0 / freq; // one full carrier period → sin = 0
        assert!(w.evaluate_at(t_full).abs() < 1e-9, "AM at full carrier period should be 0");
    }
    ...
    fn am_peak_modulator_and_carrier() {
        ...
        let vo   = 2.0;
        let va   = 1.0;
        let w = Waveform::Am { vo, va, fc, freq, td: 0.0 };
    ...
    fn trnoise_before_delay_is_zero() {
        let w = Waveform::Trnoise { na: 1.0, nt: 1e-9, nalpha: 0.0, namp: 0.0, td: 1e-6 };
        assert!(w.evaluate_at(0.0).abs() < 1e-15, "TRNOISE before td must be 0");
    }

    #[test]
    fn trnoise_zero_amplitude_is_zero() {
        let w = Waveform::Trnoise { na: 0.0, nt: 1e-9, nalpha: 0.0, namp: 0.0, td: 0.0 };
        assert!(w.evaluate_at(1e-9).abs() < 1e-15, "TRNOISE na=0 must be 0");
    }

    #[test]
    fn trnoise_same_slot_is_deterministic() {
        let w = Waveform::Trnoise { na: 1.0, nt: 1e-9, nalpha: 0.0, namp: 0.0, td: 0.0 };
    ...
    fn trnoise_different_slots_usually_differ() {
        ...
        assert!((v1 - v2).abs() > 1e-10, "different slots should give different values");
    }
    ...
    fn trrandom_before_delay_returns_mean() {
        let w = Waveform::Trrandom { kind: 1, tstep: 1e-6, td: 1e-6, param: 1.0, mean: 3.0 };
        assert!((w.evaluate_at(0.0) - 3.0).abs() < 1e-15, "before td must return mean");
    }

    #[test]
    fn trrandom_uniform_same_step_deterministic() {
        let w = Waveform::Trrandom { kind: 1, tstep: 1e-6, td: 0.0, param: 2.0, mean: 0.0 };
    ...
    fn trrandom_uniform_in_range() {
        let param = 2.0; // range
        let mean  = 5.0;
        let w = Waveform::Trrandom { kind: 1, tstep: 1e-9, td: 0.0, param, mean };
        for i in 0..20u64 {
            let t = (i as f64 + 0.5) * 1e-9;
            let v = w.evaluate_at(t);
            assert!(v >= mean - param / 2.0 - 1e-12 && v <= mean + param / 2.0 + 1e-12,
    ...
    fn trrandom_gaussian_different_steps_differ() {
        let w = Waveform::Trrandom { kind: 2, tstep: 1e-6, td: 0.0, param: 1.0, mean: 0.0 };
    ...
    fn trrandom_exponential_positive() {
        ...
        let w = Waveform::Trrandom { kind: 3, tstep: 1e-9, td: 0.0, param: 1.0, mean: 0.0 };
        for i in 0..20u64 {
            let t = (i as f64 + 0.5) * 1e-9;
            let v = w.evaluate_at(t);
            assert!(v >= 0.0, "exponential sample must be non-negative, got {v} at step {i}");
        }
    }
    ...
    fn pwl_repeat_wraps_after_last_point() {
        ...
        assert!((v - 0.5).abs() < 1e-10, "t=2.5 should wrap to 0.5 → value 0.5, got {v}");
    }
    ...
    fn pwl_repeat_second_full_period() {
        ...
        assert!((v - 1.0).abs() < 1e-10, "t=3.0 should map to peak t=1.0 → 1.0, got {v}");
    }
    ...
    fn pwl_repeat_nonzero_offset() {
        ...
        assert!((v - 5.0).abs() < 1e-10, "t=4.0 should map to peak at t=2.0 → 5.0, got {v}");
    }
    ...
    fn from_params_am() {
        ...
        assert!(w.evaluate_at(0.0).abs() < 1e-15, "AM from_params at t=0 should be 0");
    }
    ...
    fn from_params_trrandom() {
        ...
        let v = w.evaluate_at(0.5e-6);
        assert!(v.abs() <= 1.0 + 1e-12, "TRRANDOM uniform sample out of range: {v}");
    }
    ...
    fn from_params_pwl_repeat() {
        ...
        p.set("pwl_count",      3.0);
        p.set("pwl_t0",         0.0);  p.set("pwl_v0", 0.0);
        p.set("pwl_t1",         1.0);  p.set("pwl_v1", 1.0);
        p.set("pwl_t2",         2.0);  p.set("pwl_v2", 0.0);
        p.set("pwl_r",          0.0);
        let w = Waveform::from_params(&p);
        // At t=2.5 should wrap to 0.5 → value 0.5.
        let v = w.evaluate_at(2.5);
        assert!((v - 0.5).abs() < 1e-10, "PWL REPEAT from_params wrap: got {v}");
    }
}
