...
impl DeviceModel for Diode {
    fn eval(&self, voltages: &[f64], params: &ParamMap) -> DeviceEval {
        ...
        let cj0    = params.get_or("cj0", 0.0);
        let vj     = params.get_or("vj", 1.0).max(0.01); // guard against vj=0
        ...
        let vt = params.get("vt").unwrap_or_else(|| Self::KB * temp / Self::Q);
        ...
        let is_eff    = is_scaled * area;

        // Area-scaled zero-bias junction cap.
        let cj0_eff = cj0 * area;
        ...
        let (id_fwd, gd_fwd) = if vd <= vlimit {
            let exp_val = (vd / nvt).exp();
            (is_eff * (exp_val - 1.0) + GMIN * vd,
             is_eff * exp_val / nvt + GMIN)
        } else {
        ...
        // Active when Vd < -BV.  Formula:
        //   Id_bd = -IBV * exp(-(Vd + BV) / nvt)
        //   Gd_bd =  IBV / nvt * exp(-(Vd + BV) / nvt)
        let (id_bd, gd_bd) = if vd < -bv {
            let arg = -(vd + bv) / nvt;
            // Clamp to avoid overflow — same strategy as forward vlimit.
            let arg_clamped = arg.min(500.0);
            let exp_bd = arg_clamped.exp();
            (-ibv * exp_bd, ibv / nvt * exp_bd)
        } else {
            (0.0, 0.0)
        };

        // ── Combine and apply multiplicity ────────────────────────────────
        let id_total = (id_fwd + id_bd) * m_mult;
        let gd_total = (gd_fwd + gd_bd) * m_mult;
        ...
        let cj = Self::junction_cap(cj0_eff, vd, vj, mj, fc) * m_mult;
        let qj = Self::junction_charge(cj0_eff, vd, vj, mj, fc) * m_mult;
        ...
        // diodes).  The charge is Q_TT = TT * Id, so the capacitance
        // contribution is dQ_TT/dVd = TT * gd.  This stamps identically to
        // the junction cap but uses the DC conductance rather than Cj(V).
        let tt = params.get_or("tt", 0.0);
        let q_tt = tt * id_total;  // TT * Id (already multiplicity-scaled)
        let c_tt = tt * gd_total;  // TT * gd — used in C Jacobian
...
mod tests {
    ...
    fn temperature_scaling_identity_at_tnom() {
        ...
        assert!((is_out - is_nom).abs() < 1e-30, "should be identity at tnom");
    }
    ...
    fn zener_breakdown_at_exact_bv() {
        ...
        assert!(
            ...
            "At Vd=-BV breakdown is inactive; got id={}", eval.g[0]
        );
        ...
        assert!(
            eval_bd.g[0] < -ibv * 0.5,
            "Just below BV breakdown current should dominate; got id={}", eval_bd.g[0]
        );
    }
    ...
    fn transit_time_charge_stamps_q_and_c() {
        ...
        assert!(
            ...
            eval_no.g[0], eval_tt.g[0]
        );

        // q[0] with TT must equal q[0] without TT (=0, no cj0) + TT * Id.
        let expected_q_tt = tt * eval_no.g[0];
        assert!(
            (eval_tt.q[0] - expected_q_tt).abs() < 1e-28,
            "q[0] with TT={}: got {}, expected {}",
            tt, eval_tt.q[0], expected_q_tt
        );
        // Antisymmetric.
        assert!(
        ...
        // C Jacobian must have 4 entries and C[0,0] ≈ TT * gd.
        assert_eq!(eval_tt.C.len(), 4, "C must have 4 entries with TT");
        let c00 = eval_tt.C[0].2;
        let expected_c = tt * eval_no.G[0].2; // TT * gd
        assert!(
