    ...
    fn lower_func(
        ...
        match name {
            ...
            "__table__" => {
                ...
                return Ok(self.push(ExprNode::Table { x: x_idx, points_idx: table_idx }));
            }
            _ => {}
        }
    ...
    pub fn partial(
        ...
        for node in &self.nodes {
            let (vi, di) = match *node {
                ...
                ExprNode::If(c, t, e) => {
                    let val = if v[c.get()] != 0.0 { v[t.get()] } else { v[e.get()] };
                    let deriv = if v[c.get()] != 0.0 { d[t.get()] } else { d[e.get()] };
                    (val, deriv)
                }
                ExprNode::Table { x, points_idx } => {
                    let xv = v[x.get()];
...
pub fn table_lookup(points: &[(f64, f64)], x: f64) -> f64 {
    ...
    let i = match points.binary_search_by(|p| {
        p.0.partial_cmp(&x).unwrap_or(std::cmp::Ordering::Equal)
    }) {
...
mod tests {
    ...
    fn table_node_eval_and_partial() {
        ...
        let bx = BehavioralExpr::Func(
            "__table__".into(),
            vec![nv("x"), lit(0.0), lit(0.0), lit(1.0), lit(2.0), lit(2.0), lit(4.0)],
        );
