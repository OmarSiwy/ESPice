...
Wall-clock time is measured from the start of `run_bigospice_once()` to completion. This includes:
...
```bash
RUSTFLAGS="-C target-cpu=native" cargo run -p bigospice-bench-suite --release -- ...
```
...
Where `speedup_i = ngspice_time_i / bigospice_time_i` for each circuit i.
...
| Metric | Definition |
|---|---|
| `max_abs_err` | max |bigospice_v - ngspice_v| across all nodes and time points |
| `max_rel_err` | max |bigospice_v - ngspice_v| / (|ngspice_v| + 1e-12) |
| `rmse` | sqrt( mean( (bigospice_v - ngspice_v)^2 ) ) |
