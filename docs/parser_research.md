...
- **SI suffixes on numbers:** `1k` → 1000.0, `100n` → 1e-7, `2.2meg` → 2.2e6, `1G` → 1e9. Uses `bigospice_utility::si_multiplier()`.
...
Accumulated into `bigospice_core::SimOptions` struct.
...
```rust
use bigospice_parser::{SpiceParser, Lexer, parse_expression, eval_expression};
