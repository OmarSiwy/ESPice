...
Device internal states (e.g., MOSFET surface potentials, BJT carrier concentrations) are managed by `DeviceEval` in `bigospice_device`. These are **not** stored in the core `Circuit` struct — they're owned by the solver's scratch buffers.
...
The `bigospice-cache` crate provides **five layers** of incremental cache, each independently usable, orchestrated by `CacheManager`:
