...
The `incspice-io` crate consolidates all output format writers. It is deliberately decoupled from `incspice-analysis` (receives plain `&[Vec<f64>]` slices rather than analysis types), enabling format writers to be tested independently.
...
Bridges raw complex AC results (`AcData`) to column-major `Vec<Vec<f64>>` for format writers. Designed to have **no dependency on `incspice-analysis`**.
...
The `incspice-utility` crate provides data-oriented design primitives. All operate on the principle of **Struct-of-Arrays (SoA)** over Array-of-Structs (AoS) for cache-optimal hot-path iteration.
...
The `incspice-cli` binary provides the command-line interface.

### 3.1 Command-Line Interface

**Usage:** `incspice [--output <file|->] [--quiet] <netlist.sp>`
...
The CLI currently writes **plain tab-separated text** output (not rawfile/HSPICE/etc.). The `incspice-io` crate is available but not yet integrated into the CLI for format selection.
...
**Workflow:**
1. Build BigOSpice release binary (`cargo build --release -p incspice-cli`)
...
The CLI currently writes plain text output. The `incspice-io` crate (with all its format writers) is **not yet integrated** into the CLI. The format selection via `--format` flag is absent.
