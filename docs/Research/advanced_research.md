...
ecosystem: `incspice-osdi`, `incspice-digital`, `incspice-cosim`, and `incspice-compute`.
...
**Architecture flow:**
```
incspice CLI --dlopen--> bsim4.osdi (.so)
                             |
...
The existing `DeviceKind` enum in `incspice-core` is closed — adding an `Osdi` variant
...
`incspice-digital` implements Phase 4.1 of the mixed-signal plan: a data-oriented
...
`incspice-cosim` implements Phase 4.2: black-box integration with Verilator-compiled
...
User's SystemVerilog --> verilator --cc --> libdesign.so
                           ...
                           VerilatorModel::load() via dlopen
                                               |
                           incspice_cosim::DCosim (digital device)
                                               |
...
```
incspice_verilator_new       — constructor (returns opaque ctx handle)
incspice_verilator_eval      — advance one combinational pass
incspice_verilator_final     — terminate and free state
incspice_verilator_set_signal — drive input port by name
incspice_verilator_get_signal — read output port by name
```
...
```systemverilog
import "DPI-C" function real incspice_get_voltage(input string node);
```

The C wrapper calls `incspice_get_voltage` which forwards to the registered Rust
closure via `DpiBridge::call("incspice_get_voltage", node_name)`.
...
`incspice-compute` provides parallel compute backends (CPU via Rayon, GPU via wgpu)
