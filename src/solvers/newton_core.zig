...
fn validateEnv(comptime E: type) void {
    const decls = .{
        ...
        "read",      "limiting",     "assemble",     "precondBuild",
        "precondApply", "postStep",  "gateScale",    "currentRow",
    };
    inline for (decls) |d| {
        if (!@hasDecl(E, d))
...
pub fn newtonSolve(
    ...
    outer: while (iter < tol.max_iter) : (iter += 1) {
        ...
        if (beta < tol.abstol) {
            i = tid;
            while (i < n) : (i += stride) v.r[i] = 0; // dx = 0
            env.sync();
            if (iter > 0)
        ...
        // CPU: applyLimits (may clamp x) + updateStates; GPU: device limit
        ...
        const converged = iter > 0 and scaled < 1.0 and viol == 0 and
