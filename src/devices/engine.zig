...
pub fn Sink(comptime D: type, comptime device: bool, comptime skip_const: bool) type {
    ...
    return struct {
        ...
        pub inline fn tryCached(s: *Sk, id: u32, lx: *const [n_u]f64, corr: @Vector(n_u, f64), active: [n_u]bool) bool {
            ...
            const jac_pat = comptime rowPattern(D, "jac_pattern");
            const q_pat = comptime rowPattern(D, "q_pattern");
            // And the same WRITTEN-ROW mask, for the same reason: a replay that
            // kept a row `evalRange` drops would put a cached 0.0 into a plane
            // slot the full pass never touches, so the cache would stop being a
            // replay of it.
            const jac_row = comptime writtenRows(D, "jac_rows");
            const q_row = comptime writtenRows(D, "q_rows");
            inline for (0..n_u) |ru| if (comptime jac_row[ru]) if (active[ru]) {
