...
const GpuSolver = @import("gpu_solver.zig").GpuSolver;
...
pub const Simulation = struct {
    ...
    pub fn fromNetlist(arena: std.mem.Allocator, nl: types.Netlist, io: ?std.Io, config: SimConfig) !Simulation {
        ...
        if (io) |io_val| enable_par: {
            ...
            sim.par_eval = analysis.problem.ParEval.init(arena, io_val, &sim.circuit, @min(lanes, 16)) catch break :enable_par;
        }
    ...
    pub fn deinit(self: *Simulation) void {
        ...
        if (self.gpu_solver) |gs| gs.deinit();
    ...
    pub fn run(self: *Simulation) !void {
        ...
        if (self.gpu_compute) |*g| {
            if (self.gpu_solver == null) {
                self.gpu_solver = GpuSolver.init(self.arena, g, &self.circuit, kernels.megakernel);
                if (self.gpu_solver) |gs| self.circuit.gpu_hook = gs.hook();
            }
        }
        var ctx = RunCtx{
