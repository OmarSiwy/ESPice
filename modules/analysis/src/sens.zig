...
pub fn solve(
    ...
    const nopts: newton.Options = .{
        .max_iter = dc_opts.max_iter,
        .abstol = dc_opts.abstol,
        .gmin = dc_opts.gmin,
    };
    ...
    @memset(x_op, 0);
    const dc_result = try newton.solve(ckt, &ws.slv, x_op, ws.dx, ws.x_old, 0, nopts, newton.EvalHook{});
    ...
    for (params, entries) |p, *entry| {
        ...
        @memset(x_pert, 0);
        const pert_result = try newton.solve(ckt, &ws.slv, x_pert, ws.dx, ws.x_old, 0, nopts, newton.EvalHook{});
...
pub fn run(ctx: *const root.RunCtx, opts: Options) !root.Result {
    ...
    const refs = try ctx.circuit.collectParams(a);
    defer a.free(refs);
