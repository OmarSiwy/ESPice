...
pub fn analyze(
    ...
    // ponytail: SIMD copy instead of @memcpy
    {
        var i: usize = 0;
    ...
    for (0..n) |row| {
        for (0..n) |col| {
            ...
            // C: same procedure.
            for (0..n_samples) |k| {
                fft_re[k] = c_mats[k * n * n + elem_off];
                fft_im[k] = 0;
            }
            fft_mod.fft(fft_re, fft_im);
            for (0..n_samples) |m| {
                c_hat[m * n * n + elem_off] = .{
                    .re = fft_re[m] * inv_n,
                    .im = fft_im[m] * inv_n,
                };
            }
...
test "PAC: RC mixer with single tone — verify frequency translation" {
    ...
    // SIMD copy + zero
    {
        ...
        var i: usize = 0;
        while (i + W <= n_samples) : (i += W) {
            fft_re[i..][0..W].* = g_time[i..][0..W].*;
            fft_im[i..][0..W].* = zero;
        }
