...
pub fn sweep(
    ...
    const nopts: newton.Options = .{
        .max_iter = options.dc_options.max_iter,
        .abstol = options.dc_options.abstol,
        .gmin = options.dc_options.gmin,
    };
    ...
    while (temp <= options.t_stop + options.t_step * 0.5) : (temp += options.t_step) {
        ...
        @memset(x, 0);
        const dc_result = try newton.solve(ckt, &ws.slv, x, ws.dx, ws.x_old, 0, nopts, newton.EvalHook{});
...
pub fn run(ctx: *const root.RunCtx, opts: Options) !root.Result {
    ...
    const st = try sweep(ckt, x, ctx.probes, &.{}, temps, values, opts, a);
