...
pub const DeviceVtable = struct {
    ...
    proto_create: *const fn (std.mem.Allocator) anyerror!problem.Proto,
...
pub fn layoutHash() u64 {
    return comptime blk: {
        ...
        for ([_]type{
            DeviceVtable,           problem.Proto, root.Batch,
            root.Hooks,             root.Planes,   root.Circuit,
            pattern.PatternBuilder, root.ParamRef, root.NoiseSource,
            std.mem.Allocator,
        }) |T| h = hashType(h, T);
        break :blk h;
    };
...
fn Impl(comptime D: type, comptime device_name: []const u8) type {
    return struct {
        const Store = batch.ProtoStore(D);
        ...
        fn protoCreate(gpa: std.mem.Allocator) anyerror!problem.Proto {
        ...
        fn finalizeNoGpu(ctx: *anyopaque, gpa: std.mem.Allocator, ckt: *const root.Circuit) anyerror!root.Batch {
            var b = try Store.finalize(ctx, gpa, ckt);
            b.gpu_kind_id = 0;
            b.hooks = &nogpu_hooks;
            return b;
        }
        const nogpu_hooks: root.Hooks = blk: {
            var h = batch.DeviceBatch(D).hooks;
            h.gpu_pack = null;
...
test "dyn vtable: blob init, param set by name, proto add" {
    ...
    const store: *batch.ProtoStore(R) = @ptrCast(@alignCast(proto.ctx));
