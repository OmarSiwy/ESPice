...
// ============================================================================
// Parallel device evaluation (lanes over private plane slabs, reduced
// afterwards). Engine-owned policy; mechanism lives here with the rest of
    ...
    pub fn init(gpa: std.mem.Allocator, io: std.Io, ckt: *Circuit, n_lanes_req: u32) !ParEval {
        ...
        for (windows, 1..) |*win, l| {
            ...
            for (tasks.items[task_off[l]..task_off[l + 1]]) |task| {
                ...
                const bounds = b.hooks.scatter_bounds(b.ctx, task.first, task.last, ckt.trash_slot, ckt.n);
