...
//!
//! Same idea: open libamdhip64 at runtime and resolve symbols into a
//! function-pointer table, so the binary builds without ROCm present and a
//! missing runtime/GPU is a clean error. HIP symbols are NOT versioned, the
//! device pointer is opaque, and the loaded image is an AMDGCN code object.
...
const Api = struct {
    ...
    hipModuleLaunchKernel: *const fn (
        hipFunction_t,
        c_uint,
        c_uint,
        c_uint,
