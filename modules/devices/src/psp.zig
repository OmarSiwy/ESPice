...
pub const g_pattern_override = [_]contract.Entry(n_u){
    ...
    // Bulk resistance: B <-> BI
    .{ .row = @intFromEnum(U.bulk), .col = @intFromEnum(U.bulk) },
    .{ .row = @intFromEnum(U.bulk), .col = @intFromEnum(U.bi) },
    .{ .row = @intFromEnum(U.bi), .col = @intFromEnum(U.bulk) },
    .{ .row = @intFromEnum(U.bi), .col = @intFromEnum(U.bi) },
    // Intrinsic MOSFET: GP, DI, SI, BI (4x4 dense block)
    .{ .row = @intFromEnum(U.gp), .col = @intFromEnum(U.di) },
    .{ .row = @intFromEnum(U.gp), .col = @intFromEnum(U.si) },
    .{ .row = @intFromEnum(U.gp), .col = @intFromEnum(U.bi) },
    .{ .row = @intFromEnum(U.di), .col = @intFromEnum(U.gp) },
    .{ .row = @intFromEnum(U.di), .col = @intFromEnum(U.si) },
    .{ .row = @intFromEnum(U.di), .col = @intFromEnum(U.bi) },
    .{ .row = @intFromEnum(U.si), .col = @intFromEnum(U.gp) },
    .{ .row = @intFromEnum(U.si), .col = @intFromEnum(U.di) },
    .{ .row = @intFromEnum(U.si), .col = @intFromEnum(U.bi) },
    .{ .row = @intFromEnum(U.bi), .col = @intFromEnum(U.gp) },
    .{ .row = @intFromEnum(U.bi), .col = @intFromEnum(U.di) },
    .{ .row = @intFromEnum(U.bi), .col = @intFromEnum(U.si) },
    // Junction nodes BS, BD <-> BI, SI, DI
    .{ .row = @intFromEnum(U.bs), .col = @intFromEnum(U.bs) },
    .{ .row = @intFromEnum(U.bs), .col = @intFromEnum(U.bi) },
    .{ .row = @intFromEnum(U.bs), .col = @intFromEnum(U.si) },
    .{ .row = @intFromEnum(U.bi), .col = @intFromEnum(U.bs) },
    .{ .row = @intFromEnum(U.si), .col = @intFromEnum(U.bs) },
    .{ .row = @intFromEnum(U.bd), .col = @intFromEnum(U.bd) },
    .{ .row = @intFromEnum(U.bd), .col = @intFromEnum(U.bi) },
    .{ .row = @intFromEnum(U.bd), .col = @intFromEnum(U.di) },
    .{ .row = @intFromEnum(U.bi), .col = @intFromEnum(U.bd) },
    .{ .row = @intFromEnum(U.di), .col = @intFromEnum(U.bd) },
    // Thermal node DT
    .{ .row = @intFromEnum(U.dt), .col = @intFromEnum(U.dt) },
};

pub const c_pattern_override = [_]contract.Entry(n_u){
    // Intrinsic charge: GP, DI, SI, BI (4x4 dense)
    .{ .row = @intFromEnum(U.gp), .col = @intFromEnum(U.gp) },
    .{ .row = @intFromEnum(U.gp), .col = @intFromEnum(U.di) },
    .{ .row = @intFromEnum(U.gp), .col = @intFromEnum(U.si) },
    .{ .row = @intFromEnum(U.gp), .col = @intFromEnum(U.bi) },
