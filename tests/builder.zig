...
test "compile + eval: resistor divider planes" {
    var setup = try @import("analyses.zig").buildSeries(testing.allocator, td.R, .{ .dc = 10 }, 1000, .{ .r = 1000 });
    defer setup.ckt.deinit();

    // n = ground + vin + vout + branch
    try testing.expectEqual(@as(u32, 4), setup.ckt.n);
    ...
    try testing.expectApproxEqAbs(g, setup.ckt.g_vals[setup.ckt.findSlot(1, 1).?], 1e-15);
    try testing.expectApproxEqAbs(-g, setup.ckt.g_vals[setup.ckt.findSlot(1, 2).?], 1e-15);
    try testing.expectApproxEqAbs(1.0, setup.ckt.g_vals[setup.ckt.findSlot(1, 3).?], 1e-15);
    // KCL vout: -g*vin + 2g*vout
    try testing.expectApproxEqAbs(2 * g, setup.ckt.g_vals[setup.ckt.findSlot(2, 2).?], 1e-15);
    // branch: vin - v = -10 residual at x=0
    try testing.expectApproxEqAbs(1.0, setup.ckt.g_vals[setup.ckt.findSlot(3, 1).?], 1e-15);
    try testing.expectApproxEqAbs(-10.0, setup.ckt.rhs[3], 1e-15);
    // ground eq pinned
    try testing.expectApproxEqAbs(1.0, setup.ckt.g_vals[setup.ckt.diag_slots[0]], 1e-15);
}
