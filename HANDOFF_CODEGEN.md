...
| ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **data-oriented-design**       | The one real win. Its rule _"replace `HashMap<EntityId, Component>` with a dense packed array"_ is exactly what `ssa.zig`'s `defs` needed. Its checklist question _"no field wider than its max value"_ also correctly told us to **stop** — see §5.2.                                 |
...
| **zig-gpu-kernel-engineering** | Framed the emitted-code work: a 60k-line function is a register-pressure and occupancy disaster, and its "hardware does not change because the language did" framing is why we chased _function shape_ rather than micro-optimising expressions.                                       |

**The meta-lesson, which matters more than any of them:** the engine was _already_
...
`defs` was `HashMap<u64 = (place<<32|block), Value>`, documented as _"sparse because
most (place,block) pairs are undefined."_ **That claim was false.** Instrumented on
`hisimhv_va`:
...
~5% of an iteration). Worth doing because LLVM will _never_ autovectorize it — it
...
run rather than a dominator query because the emitted brace nesting is _not_ the CFG
...
_returns_ cannot be seeded `undefined`, because the return is reached from exit
...
(For _codegen_ changes the output is meant to change, so this gate does not apply —
...
- **Grow the block axis to `mir.blockCount()` instead of doubling.** Re-strides more
  often, and a re-stride transiently holds the old _and_ new buffer → peak RSS went
...
padding, and field order does not affect adjacency of element _i_ across columns.
...
CSR slot (which is _why_ the operator is `+=`); and `resid_idx` _is_ the node index.
...
- The kernel body `engine.DeviceKernel` is **33 lines**: a _raw_ kernel, one thread
...
> _"on these single-huge-eval-function models something in LLVM goes superlinear on
> the safety-check CFG — measured **443 s vs 13.6 s** for hisimhv_va"_

The −26% LOC cut attacked this and won 3.5× on NVPTX _semantic analysis_, but that
...
labelled-block reconstruction case. Reducing them means changing the _block structure_
...
would attack the _cause_: 2.1 M live defs for only 1,978 variables, because
