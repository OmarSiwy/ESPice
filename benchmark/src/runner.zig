    ...
    for (fixtures) |fx| {
        ...
        if (engine_ok) {
            ...
            if (cap.ok and !std.mem.startsWith(u8, cap.text, "{\"skip\"")) {
                res.zp_cpu_median_ns = try timedMedian(io, gpa, zp_cpu_argv, cfg.timeout, 1);
        ...
        if (engine_ok) {
            ...
            if (cap.ok and !std.mem.startsWith(u8, cap.text, "{\"skip\"")) {
                res.zp_gpu_median_ns = try timedMedian(io, gpa, zp_gpu_argv, cfg.timeout, 1);
            } else if (skipReason(cap.text)) |reason| {
        ...
        if (ngspice_ok) {
            ...
            if (runOk(io, ng_argv)) {
                res.ng_median_ns = try timedMedian(io, gpa, ng_argv, cfg.timeout, 1);
            } else {
        ...
        if (xyce_ok) {
            ...
            if (runOk(io, xyce_argv)) {
                res.xyce_median_ns = try timedMedian(io, gpa, xyce_argv, cfg.timeout, 1);
            } else {
        ...
        if (res.zp_cpu_median_ns != null) {
            const zp_cpu_argv: []const []const u8 = &.{ cfg.engine_bin, "-b", "-r", zp_cpu_raw_path, netlist };
            res.zp_cpu_rss_kb = measurePeakRss(io, gpa, zp_cpu_argv, cfg.timeout);
        }
        if (res.ng_median_ns != null) {
            const ng_argv: []const []const u8 = &.{ "ngspice", "-b", "-r", ng_raw_path, netlist };
            res.ng_rss_kb = measurePeakRss(io, gpa, ng_argv, cfg.timeout);
        }
        if (res.xyce_median_ns != null) {
            const xyce_argv: []const []const u8 = &.{ "xyce", "-b", netlist };
            res.xyce_rss_kb = measurePeakRss(io, gpa, xyce_argv, cfg.timeout);
        }
